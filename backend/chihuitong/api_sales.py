...
def command(request, actor, operation, payload, callback):
    return idempotent(actor.membership.id, operation, request.headers.get("Idempotency-Key", ""), payload, callback)


def order_projection(order):
    data = {key: getattr(order, key) for key in ["source_name", "mode", "entry", "quantity", "unit_price_cents", "total_cents", "received_cents", "status", "validity_days", "units_per_card", "product_snapshot", "version", "reason", "shipment", "refund"]}
    data.update({key: str(getattr(order, key)) for key in ["id", "organization_id", "responsible_id", "product_id", "source_brand_id"]})
...
class SalesInput(StrictSerializer):
    ...
    rows = serializers.ListField(child=serializers.JSONField(), min_length=1, max_length=1, required=False)
...
def receipt_projection(item):
    return {"id": str(item.id), "order_id": str(item.order_id), "amount_cents": item.amount_cents, "paid_at": item.paid_at.isoformat(), "payer": item.payer, "reference": item.reference, "attachment_ids": item.attachment_ids, "status": item.status, "reason": item.reason, "version": item.version}
...
def orders(request):
    ...
    if request.method == "POST":
        ...
        return Response(command(request, actor, "sales.create", data, lambda: order_projection(sales.create_order(actor, **data))), status=201)
    qs = sales.visible_orders(actor)
    if request.query_params.get("product_id"):
        qs = qs.filter(product_id=serializers.UUIDField().run_validation(request.query_params["product_id"]))
    return paginated(request, qs, order_projection, states=["pending_payment", "payment_review", "pending_approval", "issued", "rejected", "cancel_pending", "stop_pending", "cancelled", "stopped"])
...
def order_rows(request, order_id):
    ...
    return paginated(request, order.rows.all(), lambda row: {"id": str(row.id), "row_number": row.row_number, "raw": row.raw, "normalized": row.normalized, "customer_id": str(row.customer_id) if row.customer_id else None, "quantity": row.quantity}, status_field=None)
...
def order_action(request, order_id, action):
    ...
    return Response(command(request, actor, f"sales.{action}", {"order_id": str(order_id), **data}, lambda: order_projection(service(actor, order_id, **data))))
...
def receipts(request, order_id):
    ...
    if request.method == "POST":
        ...
        return Response(command(request, actor, "sales.receipt", {"order_id": str(order_id), **data}, lambda: receipt_projection(sales.submit_receipt(actor, order_id, **data))), status=201)
    return paginated(request, PurchaseReceipt.objects.filter(order=order), receipt_projection, states=["pending", "approved", "rejected"])
...
def receipt_review(request, receipt_id):
    ...
    actor.require_platform()
    return Response(command(request, actor, "sales.receipt_review", {"receipt_id": str(receipt_id), **data}, lambda: receipt_projection(sales.review_receipt(actor, receipt_id, **data))))


def card_projection(card):
    return {"id": str(card.id), "serial": str(card.serial), "customer_id": str(card.customer_id) if card.customer_id else None, "status": card.status, "frozen": card.frozen, "activated_at": card.activated_at.isoformat() if card.activated_at else None, "version": card.version}


@api_view(["GET"])
def cards(request, order_id):
    order = sales.get_order(request_actor(request), order_id)
    return paginated(request, Card.objects.filter(order=order), card_projection, states=["unclaimed", "active", "void"])
...
def freeze_card(request, card_id):
    ...
    actor.require_platform()
    return Response(command(request, actor, "card.freeze", {"card_id": str(card_id), **data}, lambda: card_projection(sales.freeze_card(actor, card_id, **data))))
...
def export_cards(request, order_id):
    actor = request_actor(request)
    order = sales.get_order(actor, order_id)
    require(request.data == {"confirmed": True}, "confirmation_required", "卡片激活凭证为敏感资料，请确认安全保管", 400)
    require(order.mode == "physical" and order.status == "issued", "invalid_export", "仅已开卡实体订单支持制卡下载")
    query = order.cards.all()
    state = request.query_params.get("status", "all")
    require(state in {"all", "unclaimed", "active", "void"}, "invalid_status", "卡片状态不合法", 400)
    if state != "all":
        query = query.filter(status=state)
    audit(actor, order, "sales.cards_exported", count=query.count(), status=state)
    return excel_response(["卡号", "推广产品外部名称", "权益来源", "激活凭证", "状态"], ((str(card.serial), order.product_snapshot["external_name"], order.source_name, card.credential if card.status == "unclaimed" and not card.frozen else "不可激活", card.status) for card in query.order_by("serial").iterator()), filename="physical-cards.xlsx")


def import_projection(batch):
    return {"id": str(batch.id), "asset_id": str(batch.asset_id), "status": batch.status, "version": batch.version, "sheets": batch.sheets, "configuration": batch.configuration, "mapping_digest": batch.mapping_digest, "total_rows": batch.total_rows, "processed_rows": batch.processed_rows, "error_rows": batch.error_rows, "total_cards": batch.total_cards, "failure_code": batch.failure_code}
...
def import_list(request):
    ...
    if request.method == "POST":
        ...
        return Response(command(request, actor, "import.create", data, lambda: import_projection(imports.create_import(actor, **data))), status=201)
    return paginated(request, imports.visible_imports(actor), import_projection, states=["queued", "mapping", "validating", "validated", "confirmed", "failed"])
...
def import_mapping(request, batch_id):
    ...
    return Response(command(request, actor, "import.mapping", {"batch_id": str(batch_id), **data}, lambda: import_projection(imports.configure_import(actor, batch_id, **data))))
...
def import_confirm(request, batch_id):
    ...
    return Response(command(request, actor, "import.confirm", {"batch_id": str(batch_id), **data}, lambda: import_projection(imports.confirm_import(actor, batch_id, **data))))


@api_view(["GET"])
def import_rows(request, batch_id):
    batch = imports.get_import(request_actor(request), batch_id)
    return paginated(request, ImportRow.objects.filter(batch=batch), lambda row: {"id": str(row.id), "row_number": row.row_number, "raw": row.raw, "normalized": row.normalized, "errors": row.errors, "status": row.status}, states=["valid", "invalid"])
...
def import_errors(request, batch_id):
    ...
    return excel_response(["原始行号", "客户姓名", "手机号", "开卡数量", "异常原因"], ((row.row_number, str(row.raw.get("name", "")), str(row.raw.get("phone", "")), str(row.raw.get("quantity", "")), "；".join(error["message"] for error in row.errors)) for row in query.iterator()), filename="import-errors.xlsx")
...
def import_formats(request):
    ...
    if request.method == "POST":
        ...
        return Response({"id": str(item.id), "name": item.name, "structure": item.structure}, status=201)
    return paginated(request, ImportFormat.objects.filter(organization=actor.organization), lambda item: {"id": str(item.id), "name": item.name, "structure": item.structure}, status_field=None)
...
def import_suggest(request, batch_id):
    ...
    if data.get("format_id"):
        stored = ImportFormat.objects.filter(pk=data["format_id"], organization=actor.organization).first()
