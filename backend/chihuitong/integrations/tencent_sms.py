...
class TencentSMS:
    ...
    def send_template(self, phone, template, parameters, *, context):
        ...
        require(
            all(values.get(key) for key in ["secret_id", "secret_key", "sdk_app_id", "region"]),
            "sms_unavailable",
            "短信服务尚未配置",
            503,
        )
        require(
            template.status == "active" and template.provider_template_id and template.sign_name,
        ...
        request = urllib.request.Request(
            "https://sms.tencentcloudapi.com/", data=raw, headers=headers, method="POST"
        )
        try:
            ...
            require(
                len(rows) == 1 and rows[0].get("Code") == "Ok",
                "sms_send_rejected",
                "短信未被服务商接受",
                503,
            )
            require(
                normalize_phone(rows[0]["PhoneNumber"]) == phone,
                "sms_response_mismatch",
                "短信响应号码不匹配",
                503,
            )
            serial = rows[0].get("SerialNo", "")
            require(
                isinstance(serial, str) and re.fullmatch(r"[A-Za-z0-9_:-]{1,160}", serial),
                "sms_bad_response",
                "短信服务未返回有效发送记录",
                503,
            )
            ...
            raise BusinessError(
                "sms_network_unknown", "短信发送结果未确定，将稍后重试", 503
            ) from exc
