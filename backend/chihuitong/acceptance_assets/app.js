(function () {
  ...
  const {Button, Input, Card, Alert, Typography, Table, Tabs, Drawer, Space, Select, Modal, Spin} = arco;
  ...
  function App() {
    ...
    const showDetails=(row)=>act(async()=>{let endpoint=row.detail_endpoint;if(!endpoint&&row.id&&section!=='/api/v1/workbench/tasks')endpoint=section+'/'+row.id;setDetail(endpoint?await api(endpoint):row);});
