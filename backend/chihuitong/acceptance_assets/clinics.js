(function(){
  ...
  C.dialogs.clinic=function({id,onClose,tab='info'}){const q=C.useQuery(base+'clinics/'+id),r=q.data;return h(C.Drawer,{title:(r?.profile.name||'门诊')+' · 管理模块',onClose},h(C.Error,{error:q.error,retry:q.reload}),h(A.Spin,{loading:q.loading,style:{width:'100%'}},r&&h('div',null,h(A.Alert,{type:'info',content:r.profile.name+' · 已发布资料与待审修改分开保存，审核退回不覆盖原资料。'}),h(A.Tabs,{defaultActiveTab:tab},h(A.Tabs.TabPane,{key:'info',title:'门诊资料'},h(A.Space,{wrap:true,className:'detail-actions'},h(A.Button,{type:'primary',onClick:()=>profileForm(r)},'维护门诊资料'),C.role==='platform'&&C.button('设置待确认时限',()=>C.action('修改待确认时限',base+'clinics/'+id+'/confirmation-hours',r.version,{hours:r.confirmation_hours},[{name:'hours',label:'待确认时限（小时）',type:'number',min:1,max:168},C.Reason])),C.role==='platform'&&C.button('变更所属渠道',()=>C.open('clinicChannel',{row:r}))),h(C.Facts,{data:r,fields:['review_status','service_status','confirmation_hours']}),h(ProfileView,{profile:r.profile})),h(A.Tabs.TabPane,{key:'review',title:'资料变更审核'},h(C.List,{path:base+'clinics/'+id+'/profile-changes',columns:[C.column('id','申请编号',280),C.column('status'),C.column('due_at'),C.column('reason')],actions:change=>C.button('对照详情',()=>C.open('profileChange',{row:change,clinic:r}))})),h(A.Tabs.TabPane,{key:'contracts',title:'合同与续签'},h(C.Contracts,{orgId:r.organization_id,canManage:C.role!=='clinic'})),h(A.Tabs.TabPane,{key:'products',title:'推广产品'},h(A.Alert,{type:'info',content:'默认显示本渠道可用的全部推广产品。未上线产品不接收新预约，不能在此修改产品规则或分配。'}),h(ClinicProducts,{clinicId:id})),C.role==='platform'&&h(A.Tabs.TabPane,{key:'members',title:'门诊账号'},h(C.Members,{orgId:r.organization_id,kind:'clinic'})),h(A.Tabs.TabPane,{key:'logs',title:'操作记录'},h(C.Logs,{type:'clinic',id}))))));};
  ...
  C.dialogs.clinicForm=function({row,draft,onClose}){
    ...
    const channels=C.useQuery(C.role==='platform'?base+'organizations?status=active&page_size=100':null);
    const owners=C.useQuery(channel && C.role!=='clinic' && C.actor.role==='admin'?base+'organizations/'+channel+'/members?status=active&page_size=100':null);
