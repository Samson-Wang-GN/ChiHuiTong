...
if (os.hostname() !== 'VM-0-12-ubuntu' || process.platform !== 'linux')
  throw new Error('仅允许在指定开发服务器构建');
...
for (const audience of ['customer', 'clinic']) {
  ...
  for (const key of Object.keys(input))
    if (!['appid', 'apiBase', 'privacyReady'].includes(key))
      throw new Error('构建配置含非公开字段');
  const config = {
    audience,
    appid: input.appid || 'touristappid',
    apiBase: input.apiBase || '',
    privacyReady: input.privacyReady === true,
  };
  if (
    config.apiBase &&
    !/^https:\/\/[a-zA-Z0-9.-]+(?::443)?(?:\/[a-zA-Z0-9_/-]*)?$/.test(config.apiBase)
  )
    throw new Error('API地址必须为无凭证的HTTPS地址');
  if (config.appid !== 'touristappid' && !/^wx[a-fA-F0-9]{16}$/.test(config.appid))
    throw new Error('AppID格式错误');
  if (config.privacyReady && (config.appid === 'touristappid' || !config.apiBase))
    throw new Error('正式隐私授权须同时配置AppID和HTTPS地址');
  ...
  json('app.json', {
    pages: routes[audience].map((p) => 'pages/' + p + '/index'),
    window: {
      navigationBarTitleText: audience === 'customer' ? '齿慧通' : '齿慧通门诊',
    ...
    tabBar: {
      ...
      list: tabs.map(([p, text]) => ({ pagePath: 'pages/' + p + '/index', text })),
    },
    style: 'v2',
    ...
    ...(audience === 'customer'
      ? {
          permission: {
            'scope.userLocation': {
              desc: '仅在您主动查找附近门诊时获取位置，用于计算门诊直线距离',
            },
          },
  ...
  write('components/qr/index.wxss', '');
  json('components/qr/index.json', { component: true });
  json('BUILD.json', {
    audience,
    accountConfigured: config.appid !== 'touristappid',
    realWechatAcceptance: false,
    source: process.env.CHT_SOURCE_COMMIT || 'isolated-workspace',
  });
}
const hashes = {};
for (const file of fs.readdirSync(destination, { recursive: true }).sort()) {
  ...
  if (fs.statSync(target).isFile())
    hashes[file] = crypto.createHash('sha256').update(fs.readFileSync(target)).digest('hex');
}
