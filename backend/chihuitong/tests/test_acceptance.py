...
class AcceptanceTests(TestCase):
    def setUp(self):
        ...
        self.config = override_settings(
            ...
            ACCEPTANCE_PROXY_TOKEN="x" * 48,
            ENVIRONMENT="test",
            PRIVATE_STORAGE=Path(self.storage.name),
            SMS_BACKEND="chihuitong.acceptance.AcceptanceSMS",
        )
        self.config.enable()
    ...
    def test_gate_rejects_direct_and_cross_origin_requests(self):
        ...
        self.assertEqual(
            gate(
                factory.post(
                    "/", HTTP_X_CHT_ACCEPTANCE="x" * 48, HTTP_ORIGIN="https://evil.invalid"
                )
            ).status_code,
            403,
        )
        with override_settings(ENVIRONMENT="production"):
            self.assertEqual(
                gate(factory.get("/", HTTP_X_CHT_ACCEPTANCE="x" * 48)).status_code, 403
            )
    ...
    def test_inbox_is_encrypted_random_expiring_and_consumed(self):
        ...
        request = RequestFactory().post(
            "/acceptance/sms", data={"phone": phone}, content_type="application/json"
        )
        ...
        with patch(
            "django.utils.timezone.now", return_value=timezone.now() + timedelta(seconds=61)
        ):
            request_code(phone, "acceptance-test")
    ...
    def test_seed_persists_scoped_scenarios_and_never_resets(self):
        guard = SimpleNamespace(
            ACCEPTANCE_ENABLED=True,
            ENVIRONMENT="test",
            DATABASES={"default": {"NAME": "chihuitong_acceptance_fixture"}},
        )
