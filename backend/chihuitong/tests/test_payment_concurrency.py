...
class PaymentConcurrencyTests(TransactionTestCase):
    ...
    def test_parallel_observation_books_money_once(self):
        ...
        with patch("chihuitong.services.payments.WeChatPay", return_value=gateway):
            attempt = payments.create_payment(self.clinic_actor, self.bill.id, version=self.bill.version,
                                              method="native", key="parallel-observe")
        data = {"appid": attempt.appid, "mchid": attempt.mchid, "out_trade_no": attempt.number,
                "trade_state": "SUCCESS", "trade_type": "NATIVE", "transaction_id": "SYNTHETIC-CONCURRENT-001",
                "amount": {"total": attempt.amount_cents, "currency": "CNY"}, "success_time": timezone.now().isoformat()}
        def run():
            close_old_connections()
            try:
