...
class LocationTests(TestCase):
    def setUp(self):
        ...
        self.address = {"province": "北京市", "city": "北京市", "district": "海淀区", "address": "合成测试路1号"}

    @override_settings(TENCENT_MAP_KEY="synthetic-map-key")
    def test_candidate_gcj_precision_and_no_profile_mutation(self):
        payload = {"status": 0, "result": {"location": {"lng": 116.3, "lat": 39.9}, "level": 9, "reliability": 8}}
        with patch("chihuitong.integrations.tencent_map.request_json", return_value=payload) as request:
            response = api_client(self.channel).post("/api/v1/clinics/geocode", {**self.address, "clinic_id": str(self.clinic.id)})
    ...
    def test_missing_configuration_and_staff_scope(self):
        ...
        self.assertEqual(api_client(self.resource).post("/api/v1/clinics/geocode", self.address).status_code, 403)
        staff = actor_fixture("clinic", "13900000066", "staff", self.clinic_actor.organization)
        self.assertEqual(api_client(staff).post("/api/v1/clinics/geocode", {**self.address, "clinic_id": str(self.clinic.id)}).status_code, 403)
        other = actor_fixture("channel", "13900000067")
        self.assertEqual(api_client(other).post("/api/v1/clinics/geocode", {**self.address, "clinic_id": str(self.clinic.id)}).status_code, 404)

    def test_fixed_origin_transport_duplicate_json_and_no_error_body(self):
        with self.assertRaises(BusinessError):
            safe_json.request_json("127.0.0.1", "/")
        with patch("urllib.request.OpenerDirector.open", return_value=io.BytesIO(b'{"status":0,"status":1}')):
            with self.assertRaises(BusinessError):
                safe_json.request_json("apis.map.qq.com", "/ws/geocoder/v1/")
        with patch("urllib.request.OpenerDirector.open", return_value=io.BytesIO(json.dumps({"status": 0}).encode())) as call:
            safe_json.request_json("apis.map.qq.com", "/ws/geocoder/v1/", query={"address": "合成地址&key=bad", "key": "synthetic"})
            self.assertIn("%26key%3Dbad", call.call_args.args[0].full_url)
        with patch("urllib.request.OpenerDirector.open", side_effect=TimeoutError("must not leak secret")):
    ...
    def test_nested_untrusted_input_returns_business_error_and_external_zero_preserved(self):
        ...
        self.assertEqual(customers.normalize_customer({**raw, "resource_customer_no": 0})["resource_customer_no"], "0")
        for field, value in [("gender", []), ("gender", {}), ("age", []), ("resource_customer_no", {})]:
            with self.subTest(field=field, value=value), self.assertRaises(BusinessError):
                customers.normalize_customer({**raw, field: value})
        with self.assertRaises(BusinessError):
            clinics.validate_profile(self.channel, {"responsible_id": []}, self.channel.organization, complete=False)
