...
from .common import RESOURCE_KINDS, Actor, advance, advisory_lock, audit, check_version
...
def validate_authorization(order):
    ...
    require(
        order.source_brand.organization_id == order.organization_id
        and order.source_brand.status == "active",
        "invalid_source",
        "来源展示名当前不可用",
    )
...
def validate_rows(rows):
    require(
        isinstance(rows, list) and 0 < len(rows) <= 10000,
        "row_limit",
        "名单须为1～10000行，可分批继续",
        400,
    )
...
def create_order(
    ...
    *,
    ...
    require(
        actor.organization.kind in RESOURCE_KINDS, "forbidden", "仅客户资源方可以提交采购订单", 403
    )
    require(
        mode in {"named", "physical"} and entry in {"single", "excel", "quantity"},
        "invalid_mode",
        "销售方式不合法",
        400,
    )
    require(
        type(unit_price_cents) is int and 0 <= unit_price_cents <= 100000000,
        "invalid_price",
        "单卡采购价必须为非负整分",
        400,
    )
    require(
        type(units_per_card) is int and 1 <= units_per_card <= 10000,
        "invalid_units",
        "每卡权益份数不合法",
        400,
    )
    responsible = Membership.objects.filter(
        pk=responsible_id or actor.membership.id,
        organization=actor.organization,
        active=True,
        account__active=True,
    ).first()
    require(
        responsible and (actor.membership.role == "admin" or responsible.id == actor.membership.id),
        "invalid_responsible",
        "请选择本机构有效业务员；业务员仅可负责本人订单",
        403,
    )
    product = Product.objects.select_for_update().filter(pk=product_id, status="active").first()
    source = SourceBrand.objects.filter(
        pk=source_brand_id, organization=actor.organization, status="active"
    ).first()
    ...
    if mode == "physical":
        require(
            entry == "quantity" and not rows and not import_batch_id,
            "physical_no_excel",
            "不记名实体卡只填写数量，不上传客户名单",
            400,
        )
        ...
        require(
            rows and len(rows) == 1 and not import_batch_id and quantity is None,
            "invalid_single",
            "单客销售必须填写一名客户，不另设总数量",
            400,
        )
        ...
        require(
            entry == "excel" and not rows and quantity is None,
            "invalid_import",
            "批量销售请使用已确认的Excel名单",
            400,
        )
        batch = (
            ...
            .filter(pk=import_batch_id, organization=actor.organization, status="confirmed")
            .first()
        )
        require(
            batch
            and (actor.membership.role == "admin" or batch.created_by_id == actor.membership.id),
            "import_unconfirmed",
            "请先完成本机构名单校验并确认列映射",
            400,
        )
        ...
        quantity = sum(row["quantity"] for row in normalized)
    require(
        type(quantity) is int and 1 <= quantity <= 100000,
        "quantity_limit",
        "单次订单开卡量为1～10万张，可分批继续",
        400,
    )
    now = timezone.now()
    order = SalesOrder.objects.create(
        ...
        units_per_card=units_per_card,
        product_snapshot=product_snapshot(product),
        source_name=source.name,
        status="pending_payment" if unit_price_cents else "pending_approval",
        submitted_at=now,
        import_batch=batch,
    )
    SalesRow.objects.bulk_create(
        [
            SalesRow(
                order=order, row_number=number, raw=raw, normalized=data, quantity=data["quantity"]
            )
            for number, raw, data in zip(row_numbers, raw_rows, normalized, strict=True)
        ]
    )
    if batch:
        link_files([str(batch.asset_id)], order)
    audit(
        actor, order, "sales.submitted", mode=mode, quantity=quantity, total_cents=order.total_cents
    )
    return order
...
@transaction.atomic
def submit_receipt(
    actor, order_id, *, amount_cents, paid_at, payer, reference, attachment_ids, version
):
    ...
    require(
        type(amount_cents) is int
        and amount_cents > 0
        and amount_cents <= order.total_cents - order.received_cents,
        "invalid_amount",
        "金额须为正且不超过剩余采购款",
        400,
    )
    ...
    receipt = PurchaseReceipt.objects.create(
        ...
        attachment_ids=[str(a.id) for a in assets],
...
def review_receipt(actor, receipt_id, *, approved, version, reason):
    ...
    require(
        receipt.status == "pending" and order.status == "payment_review",
        "invalid_state",
        "付款凭证已处理或订单状态变化",
    )
    ...
    if approved:
        ...
        require(
            not PurchaseReceipt.objects.filter(
                reference_index=receipt.reference_index, status="approved"
            ).exists(),
            "duplicate_receipt",
            "该采购款流水已确认，不可重复入账",
        )
    ...
    order.status = (
        "pending_approval" if order.received_cents == order.total_cents else "pending_payment"
    )
    advance(order, "received_cents", "status")
    audit(
        actor,
        order,
        "sales.receipt_reviewed",
        reason=reason,
        receipt_id=str(receipt.id),
...
@transaction.atomic
def approve_order(actor, order_id, *, approved, version, reason, validity_days=None, queued=False):
    actor.require_platform()
    order = get_order(actor, order_id, lock=True)
    ...
    CardRange.objects.create(
        order=order, first_number=first, last_number=first + order.quantity - 1
    )
    ...
    for row, count in allocations:
        ...
        if row:
            source, _ = CustomerSource.objects.get_or_create(
                customer=row.customer, organization=order.organization
            )
                ...
                Benefit.objects.bulk_create(
                    [
                        Benefit(
                            ...
                            total=order.units_per_card,
                            pending=order.units_per_card,
                        )
                        for card in cards
                    ]
                )
            ...
            DomainEvent.objects.bulk_create(
                [
                    DomainEvent(
                        ...
                        object_id=card.id,
                        occurred_at=now,
                        payload={"customer_id": str(row.customer_id) if row else None},
                    )
                    for card in cards
                ]
            )
    order.status, order.approved_at, order.reason, order.reviewed_by = (
        "issued",
        now,
        reason,
        actor.membership,
    ...
    audit(
        ...
        validity_days=days,
        first_number=str(first),
        last_number=str(serial - 1),
    )
    return order


@transaction.atomic
def request_approval(actor, order_id, *, approved, version, reason, validity_days=None):
    actor.require_platform()
    order = get_order(actor, order_id, lock=True)
    check_version(order, version)
    require(
...
def request_stop(actor, order_id, *, action, version, reason):
    ...
    check_version(order, version)
    require(
        action in {"cancel", "stop"} and reason.strip(),
        "invalid_action",
        "请选择取消或停止并填写原因",
        400,
    )
    require(
        order.status in {"issued", "pending_payment", "pending_approval", "issue_failed"},
        "invalid_state",
        "当前存在未决收款或处理流程，请先核对",
    )
    if action == "stop":
        require(
            order.mode == "named" and order.status == "issued",
            "invalid_stop",
            "只有已开卡记名销售可停止剩余",
        )
    else:
        require(
            not order.cards.filter(activated_at__isnull=False).exists(),
            "already_activated",
            "已有卡片领取或激活，不能整单取消",
        )
...
def review_stop(actor, order_id, *, approved, version, reason):
    ...
    check_version(order, version)
    require(
        order.status in {"cancel_pending", "stop_pending"},
        "invalid_state",
        "订单没有待审核的取消或停止申请",
    )
    ...
    if not approved:
        ...
        cards = order.cards.select_for_update().filter(
            activated_at__isnull=True, voided_at__isnull=True
        )
        now = timezone.now()
        for card in cards:
...
@transaction.atomic
def register_refund(
    actor, order_id, *, amount_cents, refunded_at, reference, attachment_ids, version, reason
):
    actor.require_platform()
    order = get_order(actor, order_id, lock=True)
    check_version(order, version)
    require(
        order.status in {"cancelled", "rejected"} and order.refund.get("status") == "pending",
        "invalid_refund",
        "订单不存在待登记退款",
    )
    require(
        type(amount_cents) is int and amount_cents == order.received_cents and amount_cents > 0,
        "invalid_amount",
        "请登记全额实际退款金额",
        400,
    )
    require(
        isinstance(refunded_at, datetime)
        and timezone.is_aware(refunded_at)
        and refunded_at <= timezone.now()
        and reference.strip()
        and reason.strip(),
        "invalid_refund",
        "请填写实际退款日期、流水及原因",
    ...
    order.refund = {
        ...
        "recorded_by": str(actor.membership.id),
    }
...
def register_shipment(
    actor, order_id, *, recipient, phone, address, carrier, tracking_number, version
):
    actor.require_platform()
    order = get_order(actor, order_id, lock=True)
    check_version(order, version)
    ...
    require(
        all(
            ...
            for value in [recipient, address, carrier, tracking_number]
        ),
        "invalid_shipment",
        "请填写完整寄送资料",
        400,
