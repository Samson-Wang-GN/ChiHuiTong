...
class WorkbenchTests(TestCase):
    ...
    def test_role_workbenches_and_employee_exclusions(self):
        ...
        for actor in [
            ...
            # Exercise the SQL union directly so a regression retains its diagnostic traceback.
            workbench.list_tasks(actor)
        ...
        detail = api_client(employee).get(
            f"/api/v1/workbench/tasks/appointment_confirmation/{item.id}"
        )
        ...
        self.assertEqual(
            ...
            .get(f"/api/v1/workbench/tasks/appointment_confirmation/{item.id}")
            .status_code,
            404,
        )
    ...
    def test_task_processed_state_tracks_real_operation_not_read(self):
        ...
        self.assertEqual(
            workbench.list_tasks(self.clinic_actor, category="appointment_confirmation")["counts"][
                "pending"
            ],
            1,
        )
        appointments.confirm(
            self.clinic_actor, item.id, scheduled_at=self.scheduled, version=item.version
        )
        after = workbench.list_tasks(
            self.clinic_actor, category="appointment_confirmation", status="processed"
        )
    ...
    def test_institution_review_and_account_cannot_be_used_before_approval(self):
        ...
        with self.assertRaises(BusinessError):
            organizations.set_organization_status(
                self.platform, org.id, status="active", version=org.version, reason="不可绕过审核"
            )
        task = workbench.task_detail(self.platform, "organization_review", org.id)
        self.assertEqual(task["status"], "pending")
        org = organizations.review_organization(
            self.platform, org.id, approved=False, version=org.version, reason="联系人需核对"
        )
        ...
        org = organizations.resubmit_organization(
            self.platform, org.id, version=org.version, reason="资料已修正"
        )
        organizations.review_organization(
            self.platform, org.id, approved=True, version=org.version, reason="核验通过"
        )
    ...
    def test_object_logs_scoped_and_actor_role_snapshot(self):
        ...
        self.assertEqual(response.status_code, 200, response.data)
        confirmed = next(
            row for row in response.data["results"] if row["action"] == "appointment.confirmed"
        )
        ...
        self.assertEqual(
            api_client(other).get(f"/api/v1/objects/appointment/{item.id}/logs").status_code, 404
        )

    def test_contract_draft_edit_freeze_and_termination(self):
        previous = ContractVersion.objects.filter(
            contract__organization=self.resource.organization
        ).first()
        data = {
            "starts_at": timezone.now(),
            "ends_at": timezone.now() + timedelta(days=365),
            "contact": previous.contact,
            "attachment_ids": previous.attachment_ids,
            "settlement_cycle": "monthly",
        }
        draft = contracts.create_version(
            self.platform, self.resource.organization.id, number=previous.contract.number, data=data
        )
        data["ends_at"] += timedelta(days=1)
        draft = contracts.edit_draft(
            self.platform, draft.id, data=data, version=draft.version, reason="修订草稿"
        )
        pending = contracts.submit_version(self.platform, draft.id, version=draft.version)
        with self.assertRaises(BusinessError):
            contracts.edit_draft(
                self.platform, draft.id, data=data, version=pending.version, reason="待审不可改"
            )
        approved = contracts.review_version(
            self.platform, pending.id, approved=True, version=pending.version, reason="核验通过"
        )
        terminated = contracts.terminate(
            self.platform, approved.id, version=approved.version, reason="合作终止"
        )
        self.assertEqual(terminated.status, "terminated")
        with self.assertRaises(BusinessError):
            contracts.current_contract(self.resource.organization.id)
        self.assertEqual(
            contracts.current_contract(self.resource.organization.id, stock=True).id, terminated.id
        )
        self.assertTrue(
            AuditEvent.objects.filter(
                object_id=previous.contract_id, action="contract.terminated"
            ).exists()
        )
