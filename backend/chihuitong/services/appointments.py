...
def validate_date(when, benefit, *, future=False):
    require(
        isinstance(when, datetime) and timezone.is_aware(when),
        "invalid_time",
        "请填写有效预约日期和时间",
        400,
    )
    ...
    require(
        timezone.localtime(benefit.activated_at).date()
        <= date
        <= timezone.localtime(benefit.expires_at).date(),
        "benefit_expired",
        "预约日期不能超过权益有效期",
    )
    if future:
...
def locked_appointment(appointment_id):
    ...
    clinic = (
        ...
        .get(pk=reference["clinic_id"])
    )
    appointment = Appointment.objects.select_for_update().get(pk=appointment_id)
    benefit = (
        Benefit.objects.select_for_update()
        .select_related("card__order", "product")
...
def notify_clinic(appointment, kind):
    ...
    Outbox.objects.get_or_create(
        kind="sms.business",
        dedup_key=f"{kind}:{appointment.id}:{appointment.version}",
        defaults={
            "payload": {
                "template": kind,
                "appointment_id": str(appointment.id),
                "phone": appointment.clinic.profile["frontdesk_phone"],
            },
...
def close_tasks(appointment, reason):
    appointment.fulfillment_tasks.exclude(status="closed").update(
        status="closed", closed_at=timezone.now(), close_reason=reason
    )


def resolve_messages(appointment):
    CustomerMessage.objects.filter(appointment=appointment, status="pending").update(
...
def message(appointment, kind):
    CustomerMessage.objects.get_or_create(
        ...
        kind=kind,
        round=appointment.restoration_round,
    )
...
def appointment_event(appointment):
    event(
        ...
        completion_source=appointment.completion_source,
    )
...
def book(customer_id, *, benefit_id, clinic_id, requested_at):
    ref = (
        Benefit.objects.filter(pk=benefit_id, customer_id=customer_id)
        .values("card_id", "card__order_id")
        .first()
    )
    ...
    require(
        clinic.service_status == "online"
        and ClinicProduct.objects.filter(
            clinic=clinic, product_id=benefit.product_id, status="online"
        ).exists(),
        "clinic_offline",
        "门诊或该推广产品暂不接受新预约",
    )
    ...
    appointment = Appointment.objects.create(
        ...
        benefit=benefit,
        clinic=clinic,
        booked_channel=clinic.channel,
        requested_at=requested_at,
        pending_deadline=timezone.now() + timedelta(hours=clinic.confirmation_hours),
        units=units,
    )
...
def confirm(actor, appointment_id, *, scheduled_at, version):
    ...
    require(
        appointment.status == "pending" and timezone.now() < appointment.pending_deadline,
        "invalid_state",
        "预约已处理或超过待确认时限",
    )
...
def release_reservation(appointment, benefit):
    require(
        appointment.reserved and benefit.reserved >= appointment.units,
        "reservation_missing",
        "原预约权益占用不存在，请核对",
    )
...
def cancel(appointment_id, *, version, reason, actor=None, customer_id=None):
    ...
    check_version(appointment, version)
    require(
        reason.strip() and appointment.status in {"pending", "success"},
        "invalid_state",
        "请填写取消原因，当前预约须未完成或取消",
    )
    require(
        appointment.status != "success" or timezone.now() < appointment.scheduled_at,
        "overdue_feedback_required",
        "预约时间已过，请使用过期预约处理，不可直接释放权益",
    )
    require(
        not appointment.patient_arrived_at and not appointment.conflict,
        "feedback_conflict",
        "存在到诊反馈，请先核对",
    )
...
def reschedule(appointment_id, *, proposed_at, version, actor=None, customer_id=None, agreed=False):
    ...
    require(
        ...
        and not appointment.conflict
        and not appointment.patient_arrived_at,
        "invalid_state",
        "当前预约不能改期",
    )
    require(
        not appointment.reschedules.filter(status="pending").exists(),
        "pending_reschedule",
        "已有待处理改期申请",
    )
    validate_date(proposed_at, benefit, future=True)
    require(proposed_at != appointment.scheduled_at, "same_time", "改期时间没有变化", 400)
    change = Reschedule.objects.create(
        appointment=appointment,
        previous_at=appointment.scheduled_at,
        proposed_at=proposed_at,
        initiated_by="clinic" if actor else "customer",
        actor=actor.membership if actor else None,
        status="approved" if actor else "pending",
        agreed=bool(actor),
    ...
    audit(
        actor,
        appointment,
        "appointment.reschedule_created",
        change_id=str(change.id),
        initiator=change.initiated_by,
    )
    if not actor:
...
def review_reschedule(actor, change_id, *, approved, version, reason):
    ...
    require(
        ...
        and appointment.status == "success"
        and timezone.now() < change.expires_at,
        "invalid_state",
        "改期申请已处理或超时",
    )
    ...
    audit(
        actor,
        appointment,
        "appointment.reschedule_reviewed",
        reason=reason,
        change_id=str(change.id),
...
def report_absent(actor, appointment_id, *, version, confirmed):
    ...
    require(
        ...
        "请确认取消预约但保留权益占用，客户须自行申请恢复",
        400,
    )
    require(
        appointment.status == "success" and appointment.scheduled_at < timezone.now(),
        "invalid_state",
        "仅已过预约时间的成功预约可反馈患者未到",
    )
    require(
        not appointment.patient_arrived_at
        and not appointment.conflict
        and not appointment.reschedules.filter(status="pending").exists(),
        "feedback_conflict",
        "存在到诊反馈、冲突或未决改期，不能直接报未到",
    )
    appointment.status, appointment.clinic_absent_at, appointment.cancellation_reason = (
        "cancelled",
        timezone.now(),
        "门诊反馈患者未到，权益仍占用",
    )
...
def customer_feedback(customer_id, appointment_id, *, arrived, version, confirmed):
    ...
    check_version(appointment, version)
    require(
        type(arrived) is bool and confirmed is True,
        "confirmation_required",
        "请确认实际到诊情况",
        400,
    )
    require(
        (appointment.completion_source == "system" or appointment.clinic_absent_at)
        and appointment.reserved
        and not appointment.restoration_pending,
        "invalid_state",
        "此预约不支持到诊确认",
    )
    require(
        not appointment.redemptions.filter(status="active").exists(),
        "already_redeemed",
        "该预约已核销",
    )
    require(
    ...
    if arrived:
        ...
        FulfillmentTask.objects.get_or_create(
            appointment=appointment,
            kind="supplement",
            round=appointment.schedule_revision,
            defaults={"status": "conflict" if appointment.conflict else "pending"},
        )
        resolve_messages(appointment)
        ...
        release_reservation(appointment, benefit)
        appointment.status, appointment.cancellation_reason = (
            "cancelled",
            "客户声明未到诊，已解除权益占用",
        )
        advance(appointment, "status", "cancellation_reason", "reserved")
...
def can_redeem(appointment, benefit):
    require(
        appointment.status == "success"
        or (appointment.status == "completed" and appointment.completion_source == "system"),
        "invalid_state",
        "预约未确认、已取消或已核销",
    )
    require(
        not appointment.restoration_pending
        and appointment.reserved
        and benefit.reserved >= appointment.units,
        "reservation_missing",
        "此预约权益已释放或处于待恢复，不能核销",
    )
    require(
        not benefit.card.frozen
        and not appointment.conflict
        and not appointment.reschedules.filter(status="pending").exists(),
        "redemption_blocked",
        "存在冻结、冲突或待处理改期，不能核销",
...
@transaction.atomic
def redeem(actor, appointment_id, *, credential, confirmed, version, quote=None):
    appointment, benefit = locked_appointment(appointment_id)
    clinic_actor(actor, appointment)
    ...
    snapshot.update(
        {
            ...
            "external_name": benefit.card.order.product_snapshot["external_name"],
    ...
    record = Redemption.objects.create(
        ...
        resource_id=benefit.source.organization_id,
        channel_id=appointment.clinic.channel_id,
        units=appointment.units,
    ...
    (
        appointment.status,
        appointment.completion_source,
        appointment.completed_at,
        appointment.reserved,
    ) = "completed", "clinic", timezone.now(), False
    ...
    audit(
        actor,
        appointment,
        "appointment.redeemed",
        redemption_id=str(record.id),
...
def reverse_redemption(actor, redemption_id, *, version, reason):
    ...
    require(
        record.settled_at is None and (not bill or bill.status != "settled"),
        "already_settled",
        "账单已结清，不能撤销核销",
    )
    if bill:
        ...
        require(
            bill.received_cents <= bill.total_cents - record.fee_cents,
            "refund_review_required",
            "撤销将使已收款超过应收，请先由平台核对，暂不能自动撤销",
        )
        ...
        audit(
            actor,
            bill,
            "bill.redemption_removed",
            reason=reason,
            redemption_id=str(record.id),
            total_cents=bill.total_cents,
        )
    record.status, record.reversed_at, record.reversed_by, record.reversal_reason = (
        "reversed",
        timezone.now(),
        actor.membership,
        reason,
    ...
    if record.previous_completion_source == "system":
        ...
        benefit.reserved += appointment.units
        appointment.status, appointment.completion_source, appointment.reserved = (
            "success",
            "",
            True,
        )
    ...
    audit(
        actor,
        appointment,
        "appointment.redemption_reversed",
        reason=reason,
        redemption_id=str(record.id),
    )
    event(
...
def restore_reversal(customer_id, appointment_id, *, version, confirmed):
    ...
    require(
        appointment.restoration_pending
        and benefit.restoring >= appointment.units
        and not appointment.redemptions.filter(status="active").exists(),
    ...
    appointment.restoration_pending, appointment.status, appointment.cancellation_reason = (
        False,
        "cancelled",
        "客户申请恢复补核销撤销权益",
    )
...
def process_deadline(appointment_id, *, now=None):
    ...
    if appointment.status == "pending" and now >= appointment.pending_deadline:
        ...
        blocked = (
            appointment.patient_arrived_at
            or appointment.conflict
            or appointment.reschedules.filter(status="pending").exists()
        )
        if now >= appointment.scheduled_at + timedelta(hours=72) and not blocked:
            appointment.status, appointment.completion_source, appointment.completed_at = (
                "completed",
                "system",
                now,
            )
            ...
            _, created = FulfillmentTask.objects.get_or_create(
                appointment=appointment, kind="overdue", round=appointment.schedule_revision
            )
            if created:
