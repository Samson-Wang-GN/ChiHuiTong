...
class SyncImportTests(TestCase):
    ...
    def test_sync_api_and_lost_response_retry_do_not_duplicate(self):
        ...
        first = self.client.post("/api/v1/imports", payload, format="json", HTTP_IDEMPOTENCY_KEY="sync-create-test")
        again = self.client.post("/api/v1/imports", payload, format="json", HTTP_IDEMPOTENCY_KEY="sync-create-test")
        ...
        first = self.client.post(url, payload, format="json", HTTP_IDEMPOTENCY_KEY="sync-mapping-test")
        again = self.client.post(url, payload, format="json", HTTP_IDEMPOTENCY_KEY="sync-mapping-test")
        ...
        stale = self.client.post(url, payload, format="json", HTTP_IDEMPOTENCY_KEY="sync-mapping-stale")
        self.assertEqual(stale.status_code, 409)
        other = api_client(actor_fixture("insurance", "13900000003"))
        self.assertEqual(other.post(url, payload, format="json", HTTP_IDEMPOTENCY_KEY="sync-other-scope").status_code, 404)

    def test_file_failure_rolls_back_creation_and_same_request_can_retry(self):
        asset = asset_for(self.actor)
        with patch("chihuitong.services.imports.open_book", side_effect=BusinessError("invalid_excel", "合成错误", 400)):
            response = self.client.post("/api/v1/imports", {"asset_id": str(asset.id)}, format="json", HTTP_IDEMPOTENCY_KEY="sync-file-retry")
    ...
    def test_bounded_matching_preserves_platform_name_conflict_and_duplicate_rules(self):
        ...
        Customer.objects.create(phone="13910000000", phone_index=digest("13910000000", purpose="phone"), name="已有姓名")
        rows = [["姓名", "手机号", "数量"]] + [[f"合成{i}", str(13910000000 + i), 1] for i in range(1001)]
    ...
    def test_concurrent_same_key_creates_only_one_completed_batch(self):
        ...
        def submit(_):
            ...
            try:
                ...
                response = client.post("/api/v1/imports", {"asset_id": str(asset.id)}, format="json", HTTP_IDEMPOTENCY_KEY="sync-concurrent-test")
