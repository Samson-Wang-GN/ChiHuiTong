...
from chihuitong.models import (
    AgreementClinic, Clinic, ClinicAgreement, ClinicCooperation, InstantRedemptionOrder,
    Membership, Organization, Product,
)
...
def visible_cooperations(actor):
    ...
    if actor.organization.kind == "clinic":
        actor.require_admin()
        return qs.filter(Q(organization=actor.organization) | Q(clinics__organization=actor.organization)).distinct()
    if actor.organization.kind == "channel":
        return qs.filter(Q(clinics__in=visible_clinics(actor)) | Q(created_by=actor.membership)).distinct()
    return qs.none()
...
def assert_edit(actor, item):
    require(full_access(actor, item), "forbidden", "此操作需要签约主体完整管理范围，请联系平台", 403)


def assert_no_payment(clinic_ids):
    require(
        not InstantRedemptionOrder.objects.filter(appointment__clinic_id__in=clinic_ids, status__in=["pending", "paid"]).exists(),
        "unresolved_payment", "门店仍有未决现付，请先核实支付及核销结果",
    )


@transaction.atomic
def create_cooperation(actor, *, name, kind, admin_name, admin_phone):
    require(actor.platform or actor.organization.kind == "channel", "forbidden", "仅平台或渠道可登记合作主体", 403)
    require(kind in {"single", "chain"} and isinstance(name, str) and 0 < len(name.strip()) <= 160, "invalid_subject", "请填写签约主体全称并选择单店或连锁", 400)
    ...
    item = ClinicCooperation.objects.create(organization=org, kind=kind, created_by=actor.membership)
    Membership.objects.create(organization=org, account=account_for(admin_phone, admin_name), role="admin", platform_created=True)
...
def attach_clinic(actor, cooperation_id, clinic_id, *, version, reason):
    ...
    require(clinic.service_status == "offline" and reason.strip(), "offline_required", "请先下线并填写门店关联原因")
    assert_no_payment([clinic.id])
    require(item.kind == "chain" or not item.clinics.exclude(pk=clinic.id).exists(), "single_store_only", "单店主体只能关联一家门店")
    require(not clinic.cooperation_id or clinic.cooperation_id == item.id, "subject_transfer_deferred", "已有签约主体的转移需另行核对存量承担，不允许直接改挂")
...
def current_agreement(item, *, at=None, stock=False):
    ...
    agreement = item.agreements.filter(status__in=["approved", "terminated"], starts_at__lte=at, reviewed_at__lte=at).order_by("-starts_at", "-revision").first()
    require(agreement, "contract_unavailable", "缺少已生效的双方合同，请完成签署及审核")
    if not stock:
        require(agreement.status == "approved" and agreement.ends_at >= at and item.status == "active" and item.organization.status == "active", "contract_inactive", "双方合同未生效、已到期或已终止")
    return agreement


def clinic_agreement(clinic, *, stock=False, at=None, product_id=None):
    if not clinic.cooperation_id:
        return current_contract(clinic.organization_id, at=at, stock=stock, channel_id=clinic.channel_id)
    if stock:
        at = at or timezone.now()
        qs = clinic.cooperation.agreements.filter(status__in=["approved", "terminated"], starts_at__lte=at, reviewed_at__lte=at, coverage__clinic=clinic)
        if product_id:
    ...
    require(agreement.coverage.filter(clinic=clinic).exists(), "clinic_not_covered", "当前双方合同未覆盖此门店，请联系平台核对")
    if product_id:
        require(str(product_id) in agreement.product_ids, "product_not_covered", "双方合同未包含此推广产品")
    return agreement
...
def assert_payment_ready():
    ...
    if settings.ACCEPTANCE_SIMULATED_EXTERNALS and settings.ENVIRONMENT != "production":
        return
    require(getattr(settings, "INSTANT_PAYMENT_ACCEPTED", False), "instant_not_ready", "现付收款尚未完成真实验收，暂不能上线")
...
def validate_data(actor, item, data):
    ...
    require(data["payment_mode"] == mode and (data["settlement_cycle"] == "" if mode == "instant" else data["settlement_cycle"] in {"weekly", "monthly"}), "invalid_payment_mode", "单店固定核销现付；连锁请选择周结或月结", 400)
    normalize_business_phone(data["contact"]["phone"])
    ids = {str(value) for value in data["clinic_ids"]}
    require(ids and len(ids) == len(data["clinic_ids"]), "coverage_required", "请选择不重复的覆盖门店", 400)
    stores = item.clinics.filter(pk__in=ids)
    require(stores.count() == len(ids), "coverage_invalid", "覆盖门店必须属于此签约主体", 400)
    require(not stores.exclude(pk__in=visible_clinics(actor).values("id")).exists(), "forbidden", "不能提交超出本人范围的合同覆盖，请平台处理", 403)
    require(item.kind == "chain" or len(ids) == 1, "single_store_only", "单店合同只覆盖一家门店", 400)
    products = {str(value) for value in data["product_ids"]}
    require(products and Product.objects.filter(pk__in=products, status="active").count() == len(products), "invalid_products", "请选择有效推广产品", 400)
    for store in stores:
...
def save_agreement(actor, cooperation_id, *, data, agreement_id=None, version=None):
    ...
    if agreement_id:
        ...
        require(not item.agreements.filter(status__in=["draft", "pending"]).exists(), "pending_agreement", "请先处理现有草稿或待审合同")
        revision = (item.agreements.order_by("-revision").values_list("revision", flat=True).first() or 0) + 1
        agreement = ClinicAgreement(cooperation=item, revision=revision, submitted_by=actor.membership)
    for key in ["number", "starts_at", "ends_at", "payment_mode", "settlement_cycle", "contact", "attachment_ids"]:
        setattr(agreement, key, data[key])
    agreement.product_ids = products
    if agreement_id:
        advance(agreement, "number", "starts_at", "ends_at", "payment_mode", "settlement_cycle", "contact", "attachment_ids", "product_ids")
        agreement.coverage.all().delete()
    else:
        agreement.save()
    AgreementClinic.objects.bulk_create([AgreementClinic(agreement=agreement, clinic_id=pk) for pk in ids])
...
def get_agreement(actor, agreement_id, *, lock=False):
    qs = ClinicAgreement.objects.select_related("cooperation__organization").filter(cooperation__in=visible_cooperations(actor))
    if lock:
        qs = qs.select_for_update(of=("self",))
...
def submit_agreement(actor, agreement_id, *, version):
    ...
    require(not item.cooperation.agreements.filter(status="pending").exists(), "pending_agreement", "已有待审核合同")
...
def record_paper(actor, agreement_id, *, version, data):
    ...
    allowed = {"outbound_carrier", "outbound_tracking", "recipient", "recipient_phone", "return_address"}
    if actor.platform:
        allowed |= {"received_at", "platform_signed_at", "return_carrier", "return_tracking", "returned_at"}
    require(not (set(data) - allowed - ({"signed_attachment_ids"} if actor.platform else set())), "forbidden", "无权修改平台收签或寄回记录", 403)
    require(item.status != "approved" or set(data) <= {"return_carrier", "return_tracking", "returned_at"}, "signed_immutable", "生效签署记录不可覆盖，只能补充寄回物流")
    ...
    dates = [datetime.fromisoformat(item.paper[k]) for k in ["received_at", "platform_signed_at", "returned_at"] if item.paper.get(k)]
    require(all(d <= timezone.now() for d in dates) and dates == sorted(dates), "invalid_paper_dates", "收件、签署、寄回时间须按顺序且不能是未来时间", 400)
...
@transaction.atomic
def review_agreement(actor, agreement_id, *, version, approved, reason, final=False):
    actor.require_platform()
    item = get_agreement(actor, agreement_id, lock=True)
    ClinicCooperation.objects.select_for_update().get(pk=item.cooperation_id)
    check_version(item, version)
    require(item.status == "pending" and reason.strip(), "invalid_state", "仅待审核合同可处理，请填写审核意见")
    if approved:
        from .files import validate_attachment_ids
        ...
        if final:
            require(item.paper.get("received_at") and item.paper.get("platform_signed_at"), "signature_required", "请先登记原件收件及平台签署时间")
            ...
            stores = list(Clinic.objects.select_for_update().filter(pk__in=clinic_ids).order_by("id"))
            assert_no_payment(clinic_ids)
            require(all(store.cooperation_id == item.cooperation_id for store in stores), "coverage_changed", "覆盖门店主体已变化，请重新提交")
            for store in stores:
                for product_id in item.product_ids:
                    current_contract(store.channel_id, product_id=product_id)
            previous = item.cooperation.agreements.filter(status__in=["approved", "terminated"]).order_by("-starts_at", "-revision").first()
            require(not previous or item.starts_at >= previous.starts_at, "backdated_contract", "不能回溯覆盖较新生效版本")
            item.status, item.reviewed_at, item.reviewed_by = "approved", timezone.now(), actor.membership
    else:
...
def terminate_agreement(actor, agreement_id, *, version, reason):
    ...
    item = get_agreement(actor, agreement_id, lock=True)
    check_version(item, version)
    require(item.status == "approved" and reason.strip(), "invalid_state", "仅生效合同可终止，请填写原因")
...
def batch_online(actor, clinic_ids, *, reason):
    ...
    actor.require_platform()
    require(reason.strip() and 0 < len(clinic_ids) <= 100, "invalid_batch", "请选择1～100家门店并填写原因", 400)
    results = []
    for clinic_id in dict.fromkeys(clinic_ids):
        try:
            with transaction.atomic():
                clinic = get_clinic(actor, clinic_id, lock=True)
                set_service_status(actor, clinic.id, status="online", version=clinic.version, reason=reason)
