...
def product_list(request):
    ...
    if request.method == "POST":
        return Response(
            catalog.product_snapshot(catalog.save_product(actor, validated(ProductInput, request))),
            status=201,
        )
...
def product_update(request, product_id):
    ...
    product = catalog.save_product(
        request_actor(request), data.pop("product"), product_id=product_id, **data
    )
...
def brand_list(request, org_id):
    ...
    if request.method == "POST":
        return Response(
            brand_projection(catalog.save_brand(actor, org_id, **validated(BrandInput, request)))
        )
    require(
        actor.platform
        or (actor.organization.id == org_id and actor.organization.kind in RESOURCE_KINDS),
        "not_found",
        "机构不存在或无权访问",
        404,
    )
    return paginated(
        request,
        SourceBrand.objects.filter(organization_id=org_id),
        brand_projection,
...
def upload(request):
    ...
    asset = files.upload_file(
        actor, data=file.read(files.MAX_FILE + 1), filename=file.name, purpose=data["purpose"]
    )
    return Response(
        {
            "id": str(asset.id),
...
def contract_projection(item):
    return {
        ...
        "status": item.status,
        "display_status": getattr(item, "display_status", None)
        or contracts.display_versions(ContractVersion.objects.filter(pk=item.pk))
        .values_list("display_status", flat=True)
        .get(),
...
def cooperation(request, org_id):
    ...
    versions = contracts.display_versions(
...
def contract_list(request, org_id):
    ...
    if request.method == "POST":
        require(
            not Organization.objects.filter(pk=org_id, kind="clinic").exists(),
    ...
    qs = contracts.display_versions(
        ContractVersion.objects.select_related("contract").filter(
...
def contract_submit(request, version_id):
    item = contracts.submit_version(
        request_actor(request), version_id, **validated(VersionInput, request)
    )
...
def contract_review(request, version_id):
    item = contracts.review_version(
        request_actor(request), version_id, **validated(ReviewInput, request)
    )
    return Response(contract_projection(item))
...
def contract_products(request, version_id):
    ...
    return paginated(
        request,
        qs,
        lambda t: {**contracts.term_snapshot(t), "product": catalog.product_snapshot(t.product)},
        states=["active", "disabled"],
    )
...
def clinic_projection(clinic):
    return {
        ...
        "profile_version": clinic.profile_version,
        "version": clinic.version,
        "review_status": clinic.review_status,
        "service_status": clinic.service_status,
        "confirmation_hours": clinic.confirmation_hours,
    }
...
def change_projection(change):
    return {
        "id": str(change.id),
        "clinic_id": str(change.clinic_id),
        "base_version": change.base_version,
        "before": change.before,
        "after": change.after,
        "status": change.status,
        "reason": change.reason,
        "due_at": change.due_at.isoformat(),
...
def clinic_list(request):
    ...
    if request.method == "POST":
        return Response(
            clinic_projection(clinics.create_clinic(actor, **validated(ClinicInput, request))),
            status=201,
        )
    ...
    return paginated(
        ...
        states=["online", "offline", "paused", "exited"],
    )
...
def profile_changes(request, clinic_id):
    ...
    if request.method == "POST":
        return Response(
            change_projection(
                clinics.submit_profile(actor, clinic_id, **validated(ProfileInput, request))
            ),
            status=201,
        )
    return paginated(
        request,
        ClinicProfileChange.objects.filter(clinic=clinic),
        change_projection,
    ...
    return Response(
        change_projection(
            clinics.review_profile(
                request_actor(request), change_id, **validated(ReviewInput, request)
            )
        )
    )
    ...
    return Response(
        clinic_projection(
            clinics.set_service_status(
                request_actor(request), clinic_id, **validated(ServiceInput, request)
            )
        )
    )
    ...
    return Response(
        clinic_projection(
            clinics.set_confirmation_hours(
                request_actor(request), clinic_id, **validated(HoursInput, request)
            )
        )
    )
    ...
    return Response(
        clinic_projection(
            clinics.change_channel(
                request_actor(request), clinic_id, **validated(ChannelInput, request)
            )
        )
    )
...
def clinic_products(request, clinic_id):
    ...
    if request.method == "POST":
        link = clinics.set_clinic_product(
            actor, clinic_id, **validated(ClinicProductInput, request)
        )
        return Response(
            {
                "id": str(link.id),
    ...
    qs = Product.objects.filter(
        contractproduct__contract_version=current, contractproduct__status="active"
    )
    ...
    qs = qs.annotate(
        clinic_state=Case(
            When(id__in=online_ids, then=Value("online")),
