...
def checked_xlsx(data):
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(data)) as archive:
            ...
            require(
                len(infos) <= 1000 and sum(i.file_size for i in infos) <= 50 * 1024 * 1024,
                "excel_limit",
                "Excel展开量超限，请拆分文件",
                400,
            )
            ...
            for info in infos:
                ...
                require(
                    "vbaproject" not in name
                    and "externallinks" not in name
                    and "embeddings" not in name,
                    "unsafe_excel",
                    "Excel包含宏、外部链接或嵌入对象，请移除后上传",
                    400,
                )
                ...
                if name.endswith((".xml", ".rels")):
                    lower = content.lower()
                    require(
                        b"<!doctype" not in lower and b"<!entity" not in lower,
                        "unsafe_excel",
                        "Excel包含不允许的XML实体",
                        400,
                    )
                    require(
                        not re.search(rb"targetmode\s*=\s*[\x22\x27]external", lower),
                        "unsafe_excel",
                        "Excel包含外部关系，请移除后上传",
                        400,
                    )
    ...
    try:
        ...
        while stack:
            ...
            if isinstance(item, DictionaryObject):
                require(
                    not blocked.intersection(item.keys()),
                    "unsafe_pdf",
                    "PDF包含脚本、外链或交互附件，请导出静态PDF",
                    400,
                )
                require(
                    str(item.get("/S", "")) not in blocked, "unsafe_pdf", "PDF包含不允许的动作", 400
                )
...
def validate_file(data, filename, purpose):
    ...
    require(
        isinstance(data, bytes) and 0 < len(data) <= MAX_FILE,
        "file_limit",
        "附件为空或超过10MB，请分文件上传",
        400,
    )
    ...
    try:
        with warnings.catch_warnings():
            ...
            with Image.open(io.BytesIO(data)) as image:
                ...
                require(
                    image.width * image.height <= 20000000,
                    "image_limit",
                    "图片像素过大，请压缩后上传",
                    400,
                )
...
def upload_file(actor, *, data, filename, purpose):
    ...
    asset = FileAsset.objects.create(
        ...
        uploaded_by=actor.account,
        original_name=Path(filename).name[:200],
        storage_key=key,
        content_type=content_type,
        size=len(safe_data),
        sha256=hashlib.sha256(safe_data).hexdigest(),
        purpose=purpose,
    )
...
def can_read_file(actor, asset):
    ...
    if drafts.exists() and actor.organization.kind == "channel":
        ...
        if actor.membership.role != "admin":
            allowed = allowed.filter(owner=actor.membership)
        if not allowed.exists():
            return False
        if not asset.links.filter(object_type="clinicagreement").exists():
            return True
    ...
    if asset.links.filter(
        object_type="clinicbill", object_id__in=visible_clinic_bills(actor).values("id")
    ).exists():
        return True
    if actor.membership.role == "admin":
        ...
        if asset.links.filter(
            object_type="partnerbill",
            object_id__in=PartnerBill.objects.filter(organization=actor.organization).values("id"),
        ).exists():
            return True
    if asset.links.filter(
    ...
    return (
        ...
        or asset.links.filter(object_type="clinic", object_id__in=clinic_ids).exists()
    )
...
def file_bytes(asset):
    ...
    require(
        path.parent == settings.PRIVATE_STORAGE and path.is_file(),
        "file_unavailable",
        "附件缺失，请重新上传或联系平台",
        409,
    )
    ...
    require(
        hashlib.sha256(raw).hexdigest() == asset.sha256,
        "file_integrity",
        "附件校验失败，请联系平台",
        409,
    )
...
def link_files(ids, obj):
    for asset_id in ids:
        AttachmentLink.objects.get_or_create(
            asset_id=asset_id, object_type=obj._meta.model_name, object_id=obj.id
        )
