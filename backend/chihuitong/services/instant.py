...
def visible_orders(actor):
    ...
    qs = InstantRedemptionOrder.objects.select_related(
        "appointment__clinic__organization", "cooperation__organization"
    )
    if actor.platform:
        return qs
...
def projection(item):
    return {
        ...
        "debtor_name": item.cooperation.organization.name,
        "status": item.status,
        ...
        "redemption_id": str(item.redemption_id) if item.redemption_id else None,
...
def create(actor, appointment_id, *, credential, confirmed, version, quote, key):
    ...
    with transaction.atomic():
        ...
        if old:
            require(
                old.instant_order_id and old.instant_order.appointment_id == appointment.id,
                "idempotency_conflict",
                "请求编号已用于其他付款",
            )
            return old.instant_order
        existing = appointment.instant_orders.filter(
        ...
        require(
            ...
            == {
                ...
                "fingerprint": appointments.fee_fingerprint(snapshot),
            },
            "fee_changed",
            "费用已变化，请重新核对",
        )
        ...
        order = InstantRedemptionOrder.objects.create(
            appointment=appointment,
            cooperation=appointment.clinic.cooperation,
            agreement_id=snapshot["clinic_contract_id"],
            created_by=actor.membership,
            amount_cents=snapshot["fee_cents"],
            snapshot=snapshot,
...
def prepare(attempt_id):
    ...
    try:
        ...
        PaymentAttempt.objects.filter(
            pk=attempt.id, status="creating", preparation_count=attempt.preparation_count
        ).update(status="unknown", error_code=exc.code)
    else:
        PaymentAttempt.objects.filter(
            pk=attempt.id, status="creating", preparation_count=attempt.preparation_count
        ).update(status="pending", gateway_payload=payload)
...
def observe(attempt_id, data):
    ...
    if state == "SUCCESS":
        ...
        if attempt.status != "success":
            ...
            require(
                not ReceiptLedger.objects.filter(reference_index=reference).exists(),
                "duplicate_receipt",
                "资金流水已使用",
            )
...
def complete(order_id):
    ...
    item = (
        InstantRedemptionOrder.objects.select_for_update()
        .select_related("created_by__organization", "created_by__account")
        .get(pk=order_id)
    )
    ...
    item.redemption = appointments.finish_redemption(
        Actor(item.created_by), appointment, benefit, item.snapshot, settled_at=item.paid_at
    )
...
def try_complete(order_id):
    try:
        ...
        InstantRedemptionOrder.objects.filter(pk=order_id, status="paid").update(
            error_code=code[:80]
        )
        logging.getLogger("chihuitong.payments").warning(
            "instant_completion_failed order=%s code=%s", order_id, code
        )
