...
class WeChatMini:
    def __init__(self, audience):
        ...
        require(
            re.fullmatch(r"wx[a-fA-F0-9]{16}", self.appid or "")
            and isinstance(self.secret, str)
            and len(self.secret) >= 16,
            "mini_not_configured",
            "此小程序的服务端授权尚未配置",
            503,
        )
        other = settings.MINI_PROGRAMS["clinic" if audience == "customer" else "customer"].get(
            "appid"
        )
        require(
            not other or other != self.appid,
            "mini_app_conflict",
            "客户端与门诊端须配置不同AppID",
            503,
        )
    ...
    def check(self, result):
        ...
        require(
            code == 0,
            "mini_authorization_failed",
            "微信授权未完成，请重新授权；持续失败请联系平台",
            503 if code in {-1, 45009, 45011, 40164, 40125} else 400,
        )
        return result
    ...
    def verify(self, login_code, phone_code):
        ...
        require(
            isinstance(openid, str) and 10 <= len(openid) <= 128,
            "mini_response",
            "微信身份返回格式异常",
            503,
        )
        ...
        result = self.check(
            request_json(
                ...
                data={"code": phone_code, "openid": openid},
            )
        )
        info = result.get("phone_info")
        require(
            isinstance(info, dict) and isinstance(info.get("watermark"), dict),
            "mini_response",
            "手机号授权数据格式异常",
            503,
        )
        ...
        require(
            info["watermark"].get("appid") == self.appid
            and type(stamp) is int
            and -30 <= time.time() - stamp <= 330,
            "mini_watermark",
            "手机号授权归属或有效期不符，请重新授权",
            400,
        )
        require(
            str(info.get("countryCode")) == "86",
            "unsupported_phone",
            "目前仅支持中国大陆手机号",
            400,
        )
        ...
        require(
            full_phone == "86" + phone or normalize_phone(full_phone) == phone,
            "mini_phone_mismatch",
            "授权手机号校验失败",
            400,
        )
