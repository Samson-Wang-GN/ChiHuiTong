...
from chihuitong.models import Account, AuditEvent, IdempotencyRecord, LoginChallenge, Membership, Organization, Session
...
class FoundationTests(TestCase):
    ...
    def test_sensitive_values_encrypted_and_roundtrip(self):
        with connection.cursor() as cursor:
            cursor.execute("SELECT phone, name FROM chihuitong_account WHERE id=%s", [self.platform.account.id])
    ...
    def test_create_organization_protected_initial_admin_and_audit(self):
        org = create_organization(self.platform, name="合成资源方", kind="bank", admin_name="合成管理员", admin_phone="13900000005")
        ...
        with self.assertRaisesMessage(BusinessError, "平台开通"):
            update_member(self.platform, member.id, role="staff", active=True, version=1, reason="测试")
        self.assertEqual(AuditEvent.objects.filter(object_id=org.id).count(), 1)
        with self.assertRaises(BusinessError):
            create_organization(self.resource, name="拒绝", kind="bank", admin_name="合成", admin_phone="13900000006")

    def test_staff_cannot_manage_accounts_or_other_organization(self):
        for actor, org in [(self.staff, self.resource.organization), (self.resource, self.channel.organization)]:
            with self.assertRaises(BusinessError):
    ...
    def test_last_admin_and_self_change(self):
        with self.assertRaisesMessage(BusinessError, "最后一个管理员"):
            update_member(self.platform, self.resource.membership.id, role="staff", active=True, version=1, reason="测试")
        with self.assertRaisesMessage(BusinessError, "自己"):
            update_member(self.resource, self.resource.membership.id, role="staff", active=True, version=1, reason="测试")
        create_member(self.resource, self.resource.organization.id, name="合成新管理", phone="13900000008", role="admin")
        member = update_member(self.platform, self.resource.membership.id, role="staff", active=True, version=1, reason="测试")
        self.assertEqual(member.version, 2)
        with self.assertRaisesMessage(BusinessError, "记录已更新"):
            update_member(self.platform, member.id, role="admin", active=True, version=1, reason="旧页")
    ...
    def test_api_auth_scope_and_unknown_field(self):
        ...
        response = client.post(url, {"name": "合成机构", "kind": "bank", "admin_name": "测试", "admin_phone": "13900000010", "status": "active"}, format="json")
    ...
    def test_forged_membership_and_disabled_membership(self):
        ...
        self.assertEqual(client.get(f"/api/v1/organizations/{self.resource.organization.id}/members").status_code, 403)
    ...
    def test_idempotency_conflict_and_failed_transaction(self):
        ...
        self.assertEqual(idempotent(actor, "test", "request-0001", {"a": 1}, lambda: {"ok": 1}), {"ok": 1})
        self.assertEqual(idempotent(actor, "test", "request-0001", {"a": 1}, lambda: self.fail("重复执行")), {"ok": 1})
    ...
    def test_member_tabs_and_pagination(self):
        response = api_client(self.resource).get(f"/api/v1/organizations/{self.resource.organization.id}/members?status=active&page_size=1")
        self.assertEqual(response.status_code, 200)
...
class ConcurrencyTests(TransactionTestCase):
    ...
    def test_simultaneous_admin_demotion_preserves_one(self):
        ...
        def run(member_id):
            close_old_connections()
            try:
                update_member(platform, member_id, role="staff", active=True, version=1, reason="并发测试")
                return "ok"
