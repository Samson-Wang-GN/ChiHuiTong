...
def mini_client(audience, phone, *, openid=None, name=None):
    ...
    if name:
        body["name"] = name
    with patch("chihuitong.integrations.wechat_mini.WeChatMini.verify", return_value={"appid": MINI_SETTINGS[audience]["appid"], "openid": openid or "openid-" + uuid.uuid4().hex, "phone": phone}):
...
class MiniTests(TestCase):
    ...
    def test_phone_authorization_customer_and_clinic_audiences_are_separate(self):
        ...
        self.assertEqual(api_client(self.clinic_actor).get("/api/v1/mini/clinic/me").status_code, 401)
    ...
    def test_employee_appointments_only_and_disabled_membership_immediate(self):
        ...
        client.credentials(HTTP_AUTHORIZATION="Bearer " + token, HTTP_X_MEMBERSHIP_ID=str(employee.membership.id))
        ...
        self.assertEqual(client.get(f"/api/v1/mini/clinic/organizations/{self.clinic_actor.organization.id}/cooperation").status_code, 403)
    ...
    def test_unknown_customer_can_authorize_without_name_but_must_complete_before_booking(self):
        ...
        activated = client.post("/api/v1/mini/customer/cards/activate", {"credential": order.cards.first().credential, "confirmed": True}, format="json", HTTP_IDEMPOTENCY_KEY="activate-new-phone")
        self.assertEqual(activated.status_code, 200, activated.data)
        booking = {"benefit_id": activated.data["id"], "clinic_id": str(self.clinic.id), "requested_at": self.scheduled.isoformat()}
        result = client.post("/api/v1/mini/customer/appointments", booking, format="json", HTTP_IDEMPOTENCY_KEY="incomplete-profile")
        self.assertEqual(result.status_code, 400, result.data)
        result = client.post("/api/v1/mini/customer/profile", {"name": "合成新客户", "age": 28}, format="json", HTTP_IDEMPOTENCY_KEY="complete-profile")
        self.assertEqual(result.status_code, 200, result.data)
        result = client.post("/api/v1/mini/customer/appointments", booking, format="json", HTTP_IDEMPOTENCY_KEY="complete-booking")
        self.assertEqual(result.status_code, 201, result.data)
        result = client.post("/api/v1/mini/customer/profile", {"phone": "13900000000"}, format="json", HTTP_IDEMPOTENCY_KEY="cannot-change-phone")
    ...
    def test_own_benefits_rules_direct_booking_detail_qr_and_other_customer_denied(self):
        ...
        self.assertEqual(other.get(f"/api/v1/mini/customer/appointments/{item.id}").status_code, 404)
        denied = other.post(f"/api/v1/mini/customer/cards/{self.card.id}/claim", {"confirmed": True}, format="json", HTTP_IDEMPOTENCY_KEY="cross-customer-claim")
    ...
    def test_72_hour_message_read_not_restore_then_customer_absence_releases(self):
        ...
        with patch("django.utils.timezone.now", return_value=self.scheduled + timedelta(hours=73)):
            ...
            response = client.post(f"/api/v1/mini/customer/appointments/{item.id}/feedback", {"arrived": False, "confirmed": True, "version": item.version}, format="json", HTTP_IDEMPOTENCY_KEY="not-arrived-restore")
            self.assertEqual(response.status_code, 200, response.data)
    ...
    def test_replay_and_identity_phone_change_fail_and_logout_revokes(self):
        ...
        verified = {"appid": MINI_SETTINGS["customer"]["appid"], "openid": "same-wechat-identity", "phone": self.customer.phone}
        with patch("chihuitong.integrations.wechat_mini.WeChatMini.verify", return_value=verified) as provider:
        ...
        _, changed = mini_client("customer", "13900000774", openid="same-wechat-identity", name="合成用户")
        self.assertEqual(changed.status_code, 409)
        client.credentials(HTTP_AUTHORIZATION="Bearer " + first.data["token"])
        self.assertEqual(client.post("/api/v1/mini/customer/logout", {}, format="json").status_code, 200)
...
class MiniGatewayTests(TestCase):
    ...
    def response(self):
        return {"errcode": 0, "phone_info": {"purePhoneNumber": "13900000988", "phoneNumber": "8613900000988", "countryCode": "86", "watermark": {"appid": MINI_SETTINGS["customer"]["appid"], "timestamp": int(time.time())}}}

    def test_authorized_phone_is_bound_to_server_verified_openid_and_stable_token(self):
        with patch("chihuitong.integrations.wechat_mini.request_json", side_effect=[{"openid": "verified-openid", "session_key": "discarded-secret"}, {"access_token": "synthetic-access-token", "expires_in": 7200}, self.response()]) as remote:
            ...
            self.assertEqual(remote.call_args_list[2].kwargs["data"], {"code": "phone-code", "openid": "verified-openid"})
    ...
    def test_wrong_app_stale_watermark_and_provider_rejection_fail_closed(self):
        for change in [{"appid": "wx3333333333333333"}, {"timestamp": int(time.time()) - 600}]:
            ...
            with patch("chihuitong.integrations.wechat_mini.request_json", side_effect=[{"openid": "verified-openid"}, {"access_token": "synthetic-token", "expires_in": 7200}, response]), self.assertRaises(BusinessError):
                wechat_mini.WeChatMini("customer").verify("login-code", "phone-code")
        with patch("chihuitong.integrations.wechat_mini.request_json", return_value={"errcode": 40029, "errmsg": "private-provider-response"}), self.assertRaises(BusinessError) as result:
            wechat_mini.WeChatMini("customer").verify("login-code", "phone-code")
        self.assertNotIn("private-provider-response", str(result.exception))

    @override_settings(MINI_PROGRAMS={"customer": {"appid": "", "secret": ""}, "clinic": {"appid": "", "secret": ""}})
