...
class ClinicBill(Entity):
    ...
    cooperation = models.ForeignKey(
        "ClinicCooperation", null=True, on_delete=models.PROTECT, related_name="bills"
    )
    ...
    class Meta:
        constraints = [
            ...
            models.UniqueConstraint(
                fields=["cooperation", "issued_on"], name="cooperation_bill_issue_date"
            ),
            models.CheckConstraint(
                condition=Q(clinic__isnull=False, cooperation__isnull=True)
                | Q(clinic__isnull=True, cooperation__isnull=False),
                name="bill_one_debtor",
            ),
        ]
...
class PaymentAttempt(Entity):
    bill = models.ForeignKey(
        ClinicBill, null=True, on_delete=models.PROTECT, related_name="payment_attempts"
    )
    instant_order = models.ForeignKey(
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                condition=Q(bill__isnull=False, instant_order__isnull=True)
                | Q(bill__isnull=True, instant_order__isnull=False),
                name="payment_one_target",
            ),
            models.UniqueConstraint(
                fields=["instant_order"],
                condition=Q(status__in=["creating", "pending", "unknown"]),
                name="one_unresolved_instant_payment",
            ),
            models.UniqueConstraint(
...
class ReceiptLedger(Entity):
    ...
    instant_order = models.ForeignKey(
        "InstantRedemptionOrder", null=True, on_delete=models.PROTECT, related_name="receipts"
    )
    amount_cents = models.PositiveBigIntegerField()
...
class InstantRedemptionOrder(Entity):
    appointment = models.ForeignKey(
        "Appointment", on_delete=models.PROTECT, related_name="instant_orders"
    )
    ...
    redemption = models.OneToOneField(
        Redemption, null=True, on_delete=models.PROTECT, related_name="instant_order"
    )
    ...
    class Meta:
        constraints = [
            ...
            models.CheckConstraint(
                condition=Q(status__in=["pending", "paid", "completed", "closed"]),
                name="instant_order_state",
            ),
        ]
