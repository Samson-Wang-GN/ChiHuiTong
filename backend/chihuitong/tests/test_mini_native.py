...
class NativeApiTests(TestCase):
    ...
    def test_booking_reschedule_scan_quote_redeem_and_reverse_through_native_endpoints(self):
        ...
        booked = self.post(
            ...
            cbase + "/appointments",
            {
                "benefit_id": str(self.benefit.id),
        ...
        confirmed = self.post(
            clinic,
            mbase + f"/appointments/{key}/confirm",
            {
        ...
        self.post(
            customer,
            cbase + f"/appointments/{key}/reschedule",
            {
                "version": confirmed["version"],
                "proposed_at": later.isoformat(),
            },
        )
        ...
        self.post(
            clinic,
            mbase + f"/reschedules/{change['id']}/review",
            {
        ...
        with patch("django.utils.timezone.now", return_value=later + timedelta(minutes=10)):
            ...
            self.post(
                clinic,
                mbase + f"/redemptions/{transaction['id']}/reverse",
                {
    ...
    def test_customer_absence_projection_and_feedback_are_actionable(self):
        ...
        with patch("django.utils.timezone.now", return_value=self.scheduled + timedelta(hours=1)):
            appointments.report_absent(
                self.clinic_actor, item.id, version=item.version, confirmed=True
            )
            customer, _ = mini_client("customer", self.customer.phone)
    ...
    def test_qr_uses_appointment_date_not_late_scan_date(self):
        item = confirmed_appointment(self)
        with patch(
            "django.utils.timezone.now", return_value=self.benefit.expires_at + timedelta(days=1)
        ):
            customer, _ = mini_client("customer", self.customer.phone)
    ...
    def test_clinic_contract_and_product_links_use_distinct_clinic_id(self):
        ...
        for state in [
            ...
            self.assertEqual(
                clinic.get(response.data["history_endpoint"], {"status": state}).status_code, 200
            )
