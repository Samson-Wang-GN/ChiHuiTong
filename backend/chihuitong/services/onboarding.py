...
def submit(actor, *, clinic, agreement, subject=None, clinic_id=None, version=None):
    if clinic_id:
        ...
        require(item.profile_version == 0 and item.cooperation_id and item.cooperation.kind == "single", "invalid_state", "只可补正尚未完成入驻的单店")
        cooperation = item.cooperation
        require(not cooperation.agreements.filter(status__in=["draft", "pending"]).exists(), "pending_application", "已有待处理申请")
    else:
        require(clinic.get("admin_name") and clinic.get("admin_phone"), "admin_required", "请填写门诊管理员信息", 400)
        require(subject and subject["kind"] == "single", "single_required", "单店联合入驻须填写单店签约主体", 400)
    ...
    data = {**agreement, "clinic_ids": [str(item.id)], "payment_mode": "instant", "settlement_cycle": ""}
...
def review(actor, agreement_id, *, version, approved, reason, final=False):
    ...
    require(contract.status == "pending" and change.status == "pending", "invalid_state", "申请已处理，请刷新")
    if not approved or final:
        clinics.review_profile(actor, change.id, approved=approved, version=change.version, reason=reason, joint=True)
    contract = cooperations.review_agreement(actor, contract.id, version=contract.version, approved=approved, reason=reason, final=final, joint=True)
    if approved and final:
        ...
        for product_id in contract.product_ids:
            ...
            link.status = "online"
            advance(link, "status")
        clinics.set_service_status(actor, clinic.id, status="online", version=clinic.version, reason=reason)
