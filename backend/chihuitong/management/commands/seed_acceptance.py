...
from chihuitong.services import (
    appointments,
    catalog,
    clinics,
    contracts,
...
def asset(actor, purpose):
    ...
    return files.upload_file(
        actor, data=output.getvalue(), filename=f"演示非真实-{purpose}.png", purpose=purpose
    )
...
def agreement(platform, actor, owner, product, start):
    ...
    item = contracts.create_version(
        actor,
        owner.id,
        number=f"DEMO-{owner.kind}",
        data={
            "starts_at": start - timedelta(days=10),
            "ends_at": start + timedelta(days=730),
            "settlement_cycle": "monthly",
            "contact": {"name": "演示合同联系人", "phone": ACCOUNTS["channel"][0]},
    ...
    return contracts.review_version(
        platform, item.id, approved=True, version=item.version, reason="合成验收审核，不是真实合同"
    )
...
def seed():
    if (
        not settings.ACCEPTANCE_ENABLED
        or settings.ENVIRONMENT == "production"
        or not settings.DATABASES["default"]["NAME"].startswith("chihuitong_acceptance")
    ):
    ...
    with patch("django.utils.timezone.now", return_value=start):
        ...
        for role, kind, name in [
            ("resource", "broker", "演示客户资源机构"),
            ("channel", "channel", "演示门诊渠道公司"),
        ]:
            phone, admin = ACCOUNTS[role]
            owner = organizations.create_organization(
                platform, name=name, kind=kind, admin_name=admin, admin_phone=phone
            )
            organizations.review_organization(
                platform, owner.id, approved=True, version=owner.version, reason="合成验收机构"
            )
            actors[role] = Actor(
                Membership.objects.select_related("organization", "account").get(organization=owner)
            )
        resource, channel = actors["resource"], actors["channel"]
        product = catalog.save_product(
            platform,
            {
                "internal_name": "演示·洁牙推广产品",
                "external_name": "舒心洁牙体验卡（演示）",
                "product_type": "service",
                "usage_rules": "仅演示：提前预约，每次消耗1份，不可兑换现金。",
                "redemption_units": 1,
                "fee_cents": 6000,
                "validity_days": 180,
                "status": "active",
        ...
        profile = {
            "name": "演示口腔门诊",
            "legal_entity": "演示医疗机构（非真实）",
            "province": "北京市",
            "city": "北京市",
            "district": "海淀区",
            "address": "演示路1号（虚构）",
            "business_hours": "09:00-18:00",
            "frontdesk_phone": ACCOUNTS["clinic"][0],
            "business_contact": "演示门诊联系人",
            "business_phone": ACCOUNTS["clinic"][0],
            "responsible_id": str(channel.membership.id),
            "business_license_ids": [str(license_image.id)],
            "medical_license_ids": [str(license_image.id)],
            "cover_id": str(cover.id),
        }
        clinic = clinics.create_clinic(
            channel,
            channel_id=channel.organization.id,
            profile=profile,
        ...
        clinics.set_service_status(
            platform, clinic.id, status="online", version=clinic.version, reason="演示门诊上线"
        )
        clinics.set_clinic_product(
            channel, clinic.id, product_id=product.id, online=True, reason="演示产品上线"
        )
    ...
    def sale(index, when, *, paid=False, physical=False, approve=True):
        with patch("django.utils.timezone.now", return_value=when):
            order = sales.create_order(
                ...
                quantity=6 if physical else None,
                rows=None
                if physical
                else [
                    {
                        "name": f"演示客户{index:02d}",
                        "phone": f"13900001{index:03d}",
                        "quantity": 2,
                        "resource_customer_no": f"DEMO-{index:03d}",
                        "age": 30 + index,
                        "occupation": "演示职业",
                    }
                ],
                unit_price_cents=1000 if paid else 0,
            )
            if approve and not paid:
                order = sales.approve_order(
                    platform, order.id, approved=True, version=order.version, reason="演示开卡"
                )
            return order
    ...
    def visit(index, when, *, complete=False, pending=False):
        ...
        with patch("django.utils.timezone.now", return_value=when):
            ...
            appt = appointments.book(
                ...
                clinic_id=clinic.id,
                requested_at=when + timedelta(hours=2),
            )
            if not pending:
                appt = appointments.confirm(
                    clinic_actor,
                    appt.id,
    ...
    with patch("django.utils.timezone.now", return_value=instant(prior_month)):
        ...
        receipt = finance.submit_receipt(
            ...
            payer="演示门诊（无真实资金）",
            reference="SYNTHETIC-RECEIPT-001",
            attachment_ids=[str(proof.id)],
