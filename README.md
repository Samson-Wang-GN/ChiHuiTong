...
2026-09-12已开始正式后端开发，位于`backend/`，采用Django5.2 LTS/DRF/PostgreSQL16。小程序与原型页面暂不修改。认证/机构、门诊/合同/产品、销售/Excel/权益三阶段已在指定服务器累计通过47项测试；预约及财务正在实施，不能视为全部后端或生产验收完成。后端范围、自主决定和逐阶段证据见[实施清单](docs/backend-plan.md)，技术边界见[ADR-0022](docs/decisions/0022-backend-monolith-and-isolated-validation.md)。后文原型阶段“尚无后端”按当时历史理解。
...
- 微信支付本期开发：代码及可执行模拟测试先完成，待主体、商户号与密钥就绪后真实联调。当前仓库尚无正式后端业务代码，原型支付演示不代表微信接口已完成；开发与真实验收分别跟踪TASK-079/080。
