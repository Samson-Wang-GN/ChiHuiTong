...
def cents(value):
    try:
        require(not isinstance(value, (float, bool)), "invalid_money", "金额必须为十进制字符串", 400)
        amount = Decimal(str(value))
        require(amount.is_finite() and 0 <= amount <= Decimal("9999999999.99"), "invalid_money", "金额超出有效范围", 400)
...
def product_snapshot(product):
    return {
        "id": str(product.id), "version": product.version, "internal_name": product.internal_name,
        "external_name": product.external_name, "status": product.status,
        "product_type": product.product_type, "usage_rules": product.usage_rules,
        "redemption_units": product.redemption_units, "fee_cents": product.fee_cents,
        "validity_days": product.validity_days,
    }


def validate_product(data):
    allowed = {"internal_name", "external_name", "product_type", "usage_rules", "redemption_units", "fee_cents", "validity_days", "status"}
    ...
    for key in ["internal_name", "external_name", "usage_rules"]:
        ...
        require(isinstance(data[key], str) and 0 < len(data[key].strip()) <= limit, "invalid_product", "产品名称及使用规则不能为空或超长", 400)
    require(data["status"] in {"active", "disabled"}, "invalid_status", "产品状态不合法", 400)
    require(data["product_type"] in {"service", "deduction", "discount"}, "invalid_type", "产品类型不合法", 400)
    for key in ["redemption_units", "validity_days", "fee_cents"]:
        minimum = 0 if key == "fee_cents" else 1
        require(type(data[key]) is int and minimum <= data[key] <= 1000000000, "invalid_quantity", "数量、天数或费用不合法", 400)
...
def save_product(actor, data, *, product_id=None, version=None, reason=""):
    ...
    ProductRevision.objects.create(product=product, revision=product.version, snapshot=product_snapshot(product))
...
def save_brand(actor, org_id, *, name, status="active", brand_id=None, version=None):
    ...
    require(isinstance(name, str) and 0 < len(name.strip()) <= 160 and status in {"active", "disabled"}, "invalid_brand", "来源名称或状态不合法", 400)
    if brand_id:
        brand = SourceBrand.objects.select_for_update().filter(pk=brand_id, organization=org).first()
