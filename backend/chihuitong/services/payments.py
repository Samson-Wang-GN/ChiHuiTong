...
def projection(attempt):
    data = {"id": str(attempt.id), "bill_id": str(attempt.bill_id), "number": attempt.number,
            "status": attempt.status, "amount_cents": attempt.amount_cents, "method": attempt.method,
            "bill_version": attempt.bill_version, "version": attempt.version,
...
def enqueue_reconciliation(attempt):
    Outbox.objects.get_or_create(kind="payment.reconcile", dedup_key=f"payment:{attempt.id}", defaults={
        "payload": {"attempt_id": str(attempt.id)}, "available_at": timezone.now() + timedelta(seconds=30)})


def create_payment(actor, bill_id, *, version, method, key, verified_openid=None, verified_appid=None):
    ...
    gateway = WeChatPay()
    if method == "jsapi":
        require(verified_openid and verified_appid == gateway.config.appid,
                "openid_required", "请从门诊小程序授权身份发起支付", 403)
    with transaction.atomic():
        ...
        if old:
            require(old.bill_id == bill.id and old.bill_version == version and old.method == method,
                    "idempotency_conflict", "同一请求编号不可用于其他付款")
            return old
        check_version(bill, version)
        require(bill.status == "open" and bill.total_cents > bill.received_cents,
                "invalid_state", "账单没有待付金额")
        require(not bill.receipts.filter(status="pending").exists(), "receipt_pending", "请先处理已提交付款凭证")
        ...
        if old:
            ...
            return old
        attempt = PaymentAttempt.objects.create(bill=bill, bill_version=bill.version,
            number=uuid.uuid4().hex, amount_cents=bill.total_cents - bill.received_cents,
            method=method, created_by=actor.membership, appid=gateway.config.appid,
            mchid=gateway.config.mchid, request_key=key, expires_at=timezone.now() + timedelta(minutes=15))
        enqueue_reconciliation(attempt)
        audit(actor, bill, "payment.created", attempt_id=str(attempt.id), amount_cents=attempt.amount_cents)
    try:
...
def validate_observation(attempt, data):
    require(isinstance(data, dict) and data.get("appid") == attempt.appid and data.get("mchid") == attempt.mchid
            and data.get("out_trade_no") == attempt.number, "payment_identity_mismatch", "支付主体或订单不匹配", 400)
    amount = data.get("amount")
    require(isinstance(amount, dict) and type(amount.get("total")) is int
            and amount["total"] == attempt.amount_cents and amount.get("currency") == "CNY",
            "payment_amount_mismatch", "支付金额或币种不匹配", 400)
    state = data.get("trade_state")
    require(state in {"SUCCESS", "NOTPAY", "CLOSED", "REFUND", "USERPAYING", "PAYERROR", "REVOKED"},
            "payment_unknown_state", "支付状态不合法", 400)
    if state == "SUCCESS":
        require(data.get("trade_type") == ("NATIVE" if attempt.method == "native" else "JSAPI")
                and isinstance(data.get("transaction_id"), str) and 1 <= len(data["transaction_id"]) <= 100,
                "payment_identity_mismatch", "支付类型或流水不合法", 400)
        try:
            paid_at = datetime.fromisoformat(data["success_time"])
            require(timezone.is_aware(paid_at) and paid_at <= timezone.now() + timedelta(minutes=5),
                    "payment_time_mismatch", "支付时间不合法", 400)
...
def payment_fields(data):
    ...
    return {key: data[key] for key in ["appid", "mchid", "out_trade_no", "amount", "trade_state",
                                      "trade_type", "transaction_id", "success_time"] if key in data}
...
def observe(attempt_id, data):
    ...
    if state == "SUCCESS":
        require(not attempt.gateway_transaction or attempt.gateway_transaction == data["transaction_id"],
                "payment_transaction_mismatch", "同一支付订单出现不同流水，请核对")
        ledger = record_funds(bill, reference_index=digest(attempt.mchid + ":" + data["transaction_id"], purpose="wechat_receipt"),
            amount_cents=attempt.amount_cents, received_at=datetime.fromisoformat(data["success_time"]),
            kind="wechat", source_id=attempt.id,
            force_anomaly="bill_version_changed" if bill.version != attempt.bill_version and attempt.status != "success" else "")
        ...
        audit(None, bill, "payment.observed", attempt_id=str(attempt.id), amount_cents=attempt.amount_cents,
...
def reconcile(attempt_id, *, close=False):
    ...
    gateway = WeChatPay()
    require(attempt.appid == gateway.config.appid and attempt.mchid == gateway.config.mchid,
            "payment_config_changed", "商户配置已改变，请核对原收款主体", 503)
    data = gateway.query(attempt.number)
    state = validate_observation(attempt, data)
    if state == "NOTPAY" and (close or (attempt.expires_at and attempt.expires_at <= timezone.now())):
...
def capture_notification(headers, raw):
    ...
    require(validate_observation(attempt, data) == "SUCCESS", "invalid_event", "非支付成功通知", 400)
    ...
    with transaction.atomic():
        ...
        notice, created = PaymentNotification.objects.get_or_create(notification_id=notification_id,
            defaults={"attempt": attempt, "payload": sanitized, "body_digest": fingerprint})
        require(notice.attempt_id == attempt.id and notice.body_digest == fingerprint,
                "payment_notification_conflict", "支付通知编号重复且内容不同", 409)
        if created:
            Outbox.objects.create(kind="payment.notification", dedup_key=notification_id,
                                  payload={"notification_id": str(notice.id)}, available_at=timezone.now())
    return notice
