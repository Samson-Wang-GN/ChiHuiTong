...
class MetricsTests(TestCase):
    ...
    def cohort(self, actor=None, **kwargs):
        return metrics.Cohort(
            actor or self.resource, date_from=self.start.date(), date_to=self.start.date(), **kwargs
        )
    ...
    def test_multi_source_global_people_dedup_and_staff_order_scope(self):
        ...
        self.assertEqual(
            (result["customers"], result["purchased"], result["activated"], result["appointments"]),
            (1, 2, 1, 1),
        )
        self.assertEqual(
            result["activation_rate"],
            {"value": "50.0", "numerator": 1, "denominator": 2, "unit": "%"},
        )
    ...
    def test_reversal_is_historical_not_current_and_negative_net(self):
        ...
        with patch("django.utils.timezone.now", return_value=reversed_at):
            appointments.reverse_redemption(
                self.clinic_actor, record.id, version=record.version, reason="合成错误核销"
            )
        ...
        points = cohort.trend(
            metric="redemptions",
            date_from=self.start.date(),
            date_to=reversed_at.date(),
            granularity="day",
            display="net",
        )["points"]
    ...
    def test_stop_deducts_cards_not_customer_and_frozen_activation_retained(self):
        ...
        result = self.cohort().summary()
        self.assertEqual(
            (result["customers"], result["purchased"], result["voided"], result["activated"]),
            (1, 1, 1, 1),
        )
    ...
    def test_api_filters_details_full_export_and_role_isolation(self):
        ...
        cards = client.get(
            "/api/v1/customer-overview/customer-cards",
            {**params, "customer_id": str(self.customer.id)},
        )
        self.assertEqual(cards.data["total"], 2)
        self.assertEqual(
            client.get(
                "/api/v1/customer-overview",
                {**params, "resource_id": str(self.other.organization.id)},
            ).status_code,
            403,
        )
        self.assertEqual(
            api_client(self.channel).get("/api/v1/customer-overview", params).status_code, 403
        )
        self.assertEqual(
            client.get("/api/v1/customer-overview", {**params, "forged": True}).status_code, 400
        )
