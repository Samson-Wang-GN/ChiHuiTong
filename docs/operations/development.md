...
2026-09-12补充：用户已专项授权受保护HTTPS验收入口，已增加独立`/chihuitong/`代理、验收服务和合成数据库，详见[验收运维](acceptance.md)。下文“不提供公网入口/永久服务”描述原型和自动测试阶段，不覆盖这项明确授权；Git仅本地、测试仅指定服务器、不改变其他业务服务的边界仍有效。

- 状态：0.30准备已完成，2026-09-12增加正式后端；本文原型测试记录保留，后端环境、命令及备份见[后端运维交接](backend.md)。
...
测试脚本核对Linux主机名和固定版本路径，复制版本后启动仅服务器loopback可见的HTTP进程，顺序执行19个脚本（0.31新增Excel列适配专项）；`--suite import`仅导入及销售2个脚本，`--suite smoke`仅3个脚本，均不能冒称全量通过。8765占用时拒绝启动，不杀死不明进程。日志包括逐脚本退出码、时长及原交付文件校验结果；失败保留完整日志，测试结束关闭本次HTTP服务。
...
后续0.31列适配已于2026-09-10完成服务器分批技术验证，报告与最终截图归属见[TASK-075](../tasks.md)；新增专项与销售最终2/2通过，旧18脚本通过记录独立保留。连续回归的预检查允许复用关闭连接的TIME_WAIT，但不启用SO_REUSEPORT；已有实际监听时仍拒绝启动，不借用或终止其他进程。业务原型尚待用户评审，不新增公网服务。
