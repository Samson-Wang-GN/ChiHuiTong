...
class CooperationTests(TestCase):
    def setUp(self):
        ...
        self.subject = ClinicCooperation.objects.create(
            organization=self.hq.organization, kind="chain", created_by=self.channel.membership
        )
        self.clinic.cooperation = self.subject
        self.clinic.save(update_fields=["cooperation"])
        now = timezone.now()
        self.agreement = ClinicAgreement.objects.create(
            submitted_by=self.channel.membership,
            cooperation=self.subject,
            number="SYNTH-001",
            revision=1,
            status="approved",
            ...
            contact={"name": "合成联系人", "phone": "13900000055"},
    ...
    def test_subject_scope_and_headquarters_no_patient_access(self):
        ...
        self.assertFalse(
            appointments.visible_appointments(self.hq).filter(pk=appointment.id).exists()
        )
        ...
        self.assertEqual(
            api_client(self.clinic_actor)
            .get(f"/api/v1/clinic-agreements/{self.agreement.id}")
            .data["signed_attachment_ids"],
            [],
        )

    def test_agreement_expiry_reminders_are_scoped_and_deduplicated(self):
        from chihuitong.models import Notification
        from chihuitong.services.scheduler import tick

        self.agreement.ends_at = timezone.now() + timedelta(days=20)
        self.agreement.save(update_fields=["ends_at"])
        tick()
        notice = Notification.objects.filter(object_id=str(self.agreement.id))
        recipients = set(notice.values_list("recipient_id", flat=True))
        self.assertIn(self.hq.membership.id, recipients)
        self.assertIn(self.clinic_actor.membership.id, recipients)
        self.assertIn(self.clinic.responsible_id, recipients)
        self.assertNotIn(self.resource.membership.id, recipients)
        total = notice.count()
        tick()
        self.assertEqual(notice.count(), total)

    def test_batch_online_reports_each_store_and_rejects_channel(self):
        with self.assertRaises(BusinessError):
            cooperations.batch_online(self.channel, [self.clinic.id], reason="测试")
        result = cooperations.batch_online(
            self.platform, [self.clinic.id, self.hq.organization.id], reason="资料齐全上线"
        )
    ...
    def test_chain_bills_keep_old_debt_and_scope_store_payments(self):
        ...
        self.assertEqual(
            finance.generate_cooperation_bill(self.subject.id, issued_on=issue).id, bill.id
        )
        with self.assertRaises(BusinessError):
        ...
        finance.record_funds(
            bill,
            reference_index="1" * 64,
    ...
    def test_instant_paid_once_completed_once_and_no_periodic_bill(self):
        ...
        with (
            patch("django.utils.timezone.now", return_value=self.scheduled + timedelta(hours=1)),
            self.captureOnCommitCallbacks(execute=True),
        ):
            quote = appointments.redemption_quote(
                self.clinic_actor, appointment.id, credential=self.card.credential
            )
            args = dict(
                credential=self.card.credential,
                confirmed=True,
        ...
        with self.assertRaises(BusinessError):
            appointments.reverse_redemption(
                self.clinic_actor,
                item.redemption_id,
                version=item.redemption.version,
                reason="错误核销",
            )
    ...
    def test_pending_blocks_appointment_and_does_not_release_on_deadline(self):
        ...
        with (
            patch("django.utils.timezone.now", return_value=self.scheduled + timedelta(hours=1)),
            patch("chihuitong.services.instant.prepare"),
        ):
            quote = appointments.redemption_quote(
                self.clinic_actor, appointment.id, credential=self.card.credential
            )
    ...
    def test_paid_failure_keeps_receipt_and_retries_without_payment(self):
        ...
        with (
            ...
            with patch(
                "chihuitong.services.instant.complete",
                side_effect=BusinessError("temporary", "需重试", 503),
            ):
                item = instant.create(
                    self.clinic_actor,
                    appointment.id,
    ...
    def test_real_instant_activation_requires_acceptance(self):
        with (
            override_settings(ACCEPTANCE_SIMULATED_EXTERNALS=False, INSTANT_PAYMENT_ACCEPTED=False),
            ...
            instant.observe(
                attempt.id,
                {
                    **data,
                    "trade_state": "SUCCESS",
                    "trade_type": "NATIVE",
                    "transaction_id": "late-synthetic-reference",
