...
from chihuitong.models import ContractPreparation, ContractPrint, ContractTemplate, Product
from . import cooperations, files
from .common import advance, advisory_lock, audit, check_version

FIELDS = {"number", "platform_name", "platform_credit_code", "subject_name", "subject_credit_code",
          "starts_at", "ends_at", "payment_mode", "settlement_cycle", "products", "stores", "contact_name", "contact_phone"}
PATTERN = re.compile(r"\{\{([a-z_]+)\}\}")


def visible(actor):
    require(actor.platform or actor.organization.kind == "channel", "forbidden", "仅平台和门诊渠道可办理待签合同", 403)
...
def template(actor, *, kind, title, body, platform_name, platform_credit_code, confirmed):
    ...
    require(confirmed is True, "confirmation_required", "请确认范本已获平台认可，可用于实际签署", 400)
    fields = set(PATTERN.findall(body))
    require(fields <= FIELDS and {"subject_name", "number", "platform_name"} <= fields,
            "invalid_template", "模板必须含合同编号、平台名称及门诊主体占位符，且不能含未知占位符", 400)
    require("{{" not in PATTERN.sub("", body) and "}}" not in PATTERN.sub("", body),
            "invalid_template", "模板占位符格式应为{{字段名}}", 400)
    advisory_lock("contract_template", kind)
    ContractTemplate.objects.filter(kind=kind, status="active").update(status="retired")
    item = ContractTemplate.objects.create(kind=kind, title=title, body=body, platform_name=platform_name,
        platform_credit_code=platform_credit_code, created_by=actor.membership)
...
def payload_files(payload):
    ...
    require(all(isinstance(profile.get(k, []), list) for k in ("business_license_ids", "medical_license_ids")),
            "invalid_payload", "资质附件必须为列表", 400)
    return profile.get("business_license_ids", []) + profile.get("medical_license_ids", []) + ([profile["cover_id"]] if profile.get("cover_id") else [])


@transaction.atomic
def save(actor, *, kind, payload, pk=None, version=None):
    visible(actor)
    require(isinstance(payload, dict) and len(json.dumps(payload)) <= 60000,
            "invalid_payload", "草稿内容过大或格式不正确", 400)
    require(set(payload) <= {"clinic", "agreement", "subject", "clinic_id", "version", "cooperation_id"},
            "invalid_fields", "草稿包含未知字段", 400)
    for key in ("clinic", "agreement", "subject"):
        require(isinstance(payload.get(key, {}), dict), "invalid_payload", "草稿字段格式不正确", 400)
    if not actor.platform:
        ...
        require(not channel or str(channel) == str(actor.organization.id), "forbidden", "不能代其他渠道保存草稿", 403)
    ...
    if pk:
        ...
        item = ContractPreparation.objects.create(owner=actor.membership, kind=kind,
            number="CHT-" + uuid.uuid4().hex.upper(), payload=payload)
...
def contract_input(payload):
    agreement = {k: v for k, v in payload.get("agreement", {}).items() if k not in {"attachment_ids", "number"}}
    profile = payload.get("clinic", {}).get("profile", {})
    subject = payload.get("subject", {})
    return {"agreement": agreement, "subject": {k: subject.get(k) for k in ("name", "credit_code")}, "cooperation_id": payload.get("cooperation_id"),
            "store": {k: profile.get(k) for k in ("name", "legal_entity", "province", "city", "district", "address")}}
...
def snapshot(actor, item, tpl):
    ...
    if item.payload.get("cooperation_id"):
        subject = cooperations.get_cooperation(actor, item.payload["cooperation_id"])
    ...
    require(subject_name and re.fullmatch(r"[0-9A-HJ-NPQRTUWXY]{18}", credit or ""),
            "subject_required", "请填写签约主体全称及18位统一社会信用代码", 400)
    require(data["payment_mode"] == ("instant" if item.kind == "single" else "postpaid"),
            "invalid_mode", "合作类型与付款方式不匹配", 400)
    require(data["settlement_cycle"] in ([""] if item.kind == "single" else ["weekly", "monthly"]),
            "invalid_cycle", "请核对结算周期", 400)
    ...
    if item.kind == "single":
        ...
        stores = p.get("name", "") + " " + "".join(p.get(k, "") for k in ("province", "city", "district", "address"))
        require(p.get("name") and p.get("address"), "store_required", "请填写门诊名称和经营地址", 400)
    else:
        clinics = visible_clinics(actor).filter(pk__in=data["clinic_ids"]).order_by("id")
        require(clinics.count() == len(set(data["clinic_ids"])), "forbidden", "包含无权签约的门店", 403)
        require(clinics.exists(), "coverage_required", "生成合同前请选择覆盖门店", 400)
        stores = "；".join(c.organization.name + " " + c.profile.get("address", "") for c in clinics)
    values = {"number": item.number, "platform_name": tpl.platform_name, "platform_credit_code": tpl.platform_credit_code,
              "subject_name": subject_name, "subject_credit_code": credit, "starts_at": timezone.localtime(data["starts_at"]).strftime("%Y-%m-%d %H:%M"),
              "ends_at": timezone.localtime(data["ends_at"]).strftime("%Y-%m-%d %H:%M"),
              "payment_mode": "核销现付" if item.kind == "single" else "账单后付",
              "settlement_cycle": {"": "不适用", "weekly": "周结", "monthly": "月结"}[data["settlement_cycle"]],
              "products": "；".join(f"{p.internal_name}（单次获客费{Decimal(p.fee_cents)/100:.2f}元）" for p in products),
              "stores": stores, "contact_name": data["contact"]["name"], "contact_phone": data["contact"]["phone"]}
    body = PATTERN.sub(lambda match: values[match.group(1)], tpl.body)
    return {"values": values, "body": body, "title": tpl.title, "template_id": str(tpl.id), "input": contract_input(item.payload)}
...
def pdf_bytes(data, revision):
    ...
    output = io.BytesIO()
    doc = SimpleDocTemplate(output, pagesize=A4, rightMargin=48, leftMargin=48, topMargin=58, bottomMargin=54)
    normal = ParagraphStyle("contract", fontName="ContractCJK", fontSize=10.5, leading=19, wordWrap="CJK", spaceAfter=10)
    title = ParagraphStyle("title", parent=normal, fontSize=17, leading=25, alignment=1, spaceAfter=20)
...
def generate(actor, pk, *, version):
    item = get(actor, pk, lock=True)
    editable(item, version)
    tpl = ContractTemplate.objects.filter(kind=item.kind, status="active").order_by("-created_at").first()
    require(tpl, "template_missing", "平台尚未配置正式合同模板；草稿已保留，请联系平台配置后再生成", 409)
    ...
    asset = files.upload_file(actor, data=pdf_bytes(data, revision), filename=f"{item.number}-v{revision}.pdf", purpose="contract")
    ContractPrint.objects.create(preparation=item, template=tpl, revision=revision, snapshot=data, fingerprint=checksum, asset=asset)
...
def current_print(actor, item):
    ...
    require(printed.fingerprint == fingerprint(snapshot(actor, item, printed.template)),
            "print_stale", "合同内容或产品费用已变化，请重新生成、打印并签署", 409)
    return printed
...
def sign(actor, pk, *, version, generation, attachment_ids):
    ...
    require(str(printed.asset_id) not in attachment_ids, "unsigned_file", "不能用未签署的系统生成文件代替签署照片", 400)
    ...
    audit(actor, item, "contract_draft.signed_uploaded", generation=generation)
    return item
...
def submit(actor, pk, *, version):
    ...
    editable(item, version)
    printed = current_print(actor, item)
    require(item.signed_ids and item.signed_generation == printed.revision,
            "signature_required", "请先上传当前生成版本对应的门诊签署件", 409)
    payload = {**item.payload, "agreement": {**item.payload["agreement"], "number": item.number, "attachment_ids": item.signed_ids}}
    if item.kind == "single":
        ...
        serializer = StartInput(data={"data": payload["agreement"], **{k: v for k, v in payload.items() if k in {"subject", "cooperation_id"}}})
        serializer.is_valid(raise_exception=True)
    ...
    audit(actor, item, "contract_draft.submitted", agreement_id=str(result.id), generation=printed.revision)
    return item
