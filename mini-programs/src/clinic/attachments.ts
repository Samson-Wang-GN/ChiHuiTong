...
export async function exportBill(id:string): Promise<void> {
  ...
  if (!await R.confirm('下载账单全部交易明细，包含客户资料，请妥善保管，勿向无关人员转发。')) return;
...
export async function uploadReceipt(): Promise<string> {
  ...
  R.configured(); const revision=R.epoch();
  const header = R.headers(); delete header['Content-Type'];
  return new Promise((resolve,reject)=>wx.uploadFile({url:R.endpoint('/files'),filePath:file.tempFilePath,name:'file',formData:{purpose:'payment'},header,timeout:30000,
    success:r=>{try {if(revision!==R.epoch()) throw new Error('身份已改变'); const data=JSON.parse(r.data);if(r.statusCode!==201) throw new Error(data.message||"凭证上传失败，请重新选择");resolve(data.id);}catch(e){reject(e);}},fail:()=>reject(new Error('凭证上传失败，请重试'))}));
}
