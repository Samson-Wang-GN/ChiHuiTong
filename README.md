...
最新REQ-051（2026-09-16）：应用`675696e`已部署，204项后端及四角色业务/合同、真实HTTPS验证通过。门诊开通支持保存待签草稿、自动生成PDF、下载打印、签署照片回传再提交审核；统一合同仅门诊/客户资源方/门诊渠道三类。平台先在“合同管理→合同模板配置”发布认可的单店/连锁模板；无模板可保存草稿但不能生成或提交。验证、部署和正式范本待验收边界见[合同生成交付记录](docs/contract-generation-review.md)、[TASK-110](docs/tasks.md)及[ADR-0033](docs/decisions/0033-generated-bilateral-clinic-contracts.md)。

上一轮（2026-09-16）：REQ-048～050正式后台已开发并部署应用`9b737ec`。统一合同管理、单店资料与合同一次提交/审核上线、连锁分步入驻及总部周/月账单、单店现付核销和恢复已接入真实服务端；197项后端测试及完整四角色Web回归通过。发布前后1098条核心记录ID摘要一致。现付错误核销退款延期，小程序本轮不改，真实微信商户/真机支付及生产高风险评审仍待完成。操作路径、证据和回退边界见[该轮交付记录](docs/contract-onboarding-review.md)，下方版本均为历史发布。

TASK-097～100（2026-09-13历史交付）：两个正式微信原生小程序代码位于`mini-programs/`，独立客户14页/门诊25页；当时只完成代码与服务器可执行测试，真实微信授权、原生编译/真机和发布待接入，详见[小程序开发交接](docs/mini-program-development.md)。该阶段未替换Web应用；最新Web部署以上方REQ-051为准，小程序尚未适配新模式。

TASK-096（2026-09-13）：应用df52809已部署，门诊资料区分生效位置与待审核新位置，审核对照使用真实地图而非仅坐标。175项后端测试、四角色隔离Web回归及三角色真实HTTPS地图检查通过；门诊资料与审核申请发布前后原样保留，详见[地图页面交付](docs/clinic-map-review.md)。下方版本为历史发布记录。
...
本期按用户要求启用显式非生产模拟支付/短信：`CHT_ACCEPTANCE_SIMULATED_EXTERNALS=true`仅在独立验收环境有效；支付通过真实业务账本返回模拟成功，短信记录模拟受理，不发起实际微信付款或短信发送。生产和真实支付启用时拒绝此开关，不能自动降级模拟。2026-09-13已配置本项目腾讯地图Key，用户分配额度后通过真实地址解析、底图选点确认及资源方越权验证；未提交门诊资料变更，见[接入记录](docs/tencent-map-acceptance.md)。
