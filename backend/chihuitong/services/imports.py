...
ALIASES = {
    ...
    "resource_customer_no": ["资源方客户编号", "客户编号", "客户编码", "会员编号", "客户号", "customerno"],
...
def create_import(actor, asset_id):
    ...
    batch = ImportBatch.objects.create(organization=actor.organization, created_by=actor.membership, asset=asset, status="queued")
    Outbox.objects.create(kind="excel.inspect", dedup_key=f"excel.inspect:{batch.id}:1", payload={"batch_id": str(batch.id), "version": batch.version}, available_at=timezone.now())
...
def inspect_import(batch_id, expected_version):
    ...
    try:
        for sheet in book:
            require((sheet.max_row or 0) <= 10020 and (sheet.max_column or 0) <= 200, "excel_limit", "单表超过10000条数据或200列，请拆分上传", 400)
            sheets.append({"name": sheet.title, "rows": sheet.max_row or 0, "columns": sheet.max_column or 0})
            preview[sheet.title] = [[safe_cell(cell) for cell in row] for row in sheet.iter_rows(max_row=min(20, sheet.max_row or 0))]
    finally:
        book.close()
...
@transaction.atomic
def configure_import(actor, batch_id, *, sheet, header_row, mapping, quantity_mode, uniform_quantity=None, version):
    batch = get_import(actor, batch_id, lock=True)
    require(not actor.platform, "forbidden", "导入列映射由资源方确认", 403)
    check_version(batch, version)
    require(not SalesOrder.objects.filter(import_batch=batch).exists(), "import_consumed", "已提交订单的导入依据不可修改")
    require(batch.status in {"mapping", "validated", "confirmed", "failed", "validating"}, "invalid_state", "请等待工作簿读取完成")
    sheet_meta = next((item for item in batch.sheets if item["name"] == sheet), None)
    require(sheet_meta and type(header_row) is int and 1 <= header_row <= min(20, sheet_meta["rows"]), "invalid_header", "请选择工作表和前20行内的表头", 400)
    require(isinstance(mapping, dict) and not (set(mapping) - set(ALIASES)) and {"name", "phone"} <= set(mapping), "invalid_mapping", "请对应客户姓名和手机号列", 400)
    indexes = list(mapping.values())
    require(all(type(i) is int and 0 <= i < sheet_meta["columns"] for i in indexes) and len(indexes) == len(set(indexes)), "duplicate_mapping", "不同字段不能对应同一列或不存在的列", 400)
    require(quantity_mode in {"column", "uniform"}, "quantity_required", "请选择按列读取或明确统一数量", 400)
    if quantity_mode == "column":
        require("quantity" in mapping and uniform_quantity is None, "quantity_required", "按列读取须对应开卡数量，不叠加统一值", 400)
    else:
        require(type(uniform_quantity) is int and 1 <= uniform_quantity <= 100000 and "quantity" not in mapping, "quantity_required", "请明确填写每人正整数张数，不同时选择数量列", 400)
    batch.configuration = {"sheet": sheet, "header_row": header_row, "mapping": mapping, "quantity_mode": quantity_mode, "uniform_quantity": uniform_quantity}
    batch.mapping_digest = hashlib.sha256(json.dumps({"sha256": batch.asset.sha256, **batch.configuration}, sort_keys=True).encode()).hexdigest()
    ...
    advance(batch, "configuration", "mapping_digest", "confirmed_at", "status", "processed_rows", "total_rows", "error_rows", "total_cards", "failure_code")
    Outbox.objects.create(kind="excel.validate", dedup_key=f"excel.validate:{batch.id}:{batch.version}", payload={"batch_id": str(batch.id), "version": batch.version}, available_at=timezone.now())
...
def validate_import(batch_id, expected_version):
    ...
    try:
        ...
        for number, cells in enumerate(sheet.iter_rows(min_row=config["header_row"] + 1), start=config["header_row"] + 1):
            ...
            errors, normalized = [], {}
            try:
                require(not any(cells[index].data_type == "f" for index in config["mapping"].values()), "formula_field", "关键字段包含公式，请转换为值后上传", 400)
            ...
            values.append(ImportRow(batch=batch, row_number=number, raw=raw, normalized=normalized, errors=errors, status="invalid" if errors else "valid"))
    finally:
        book.close()
    for row in values:
        if row.normalized.get("phone") in duplicate_phones:
            row.errors.append({"code": "duplicate_phone", "message": "同一名单存在重复手机号，请核对"})
            row.status = "invalid"
    ...
    with transaction.atomic():
        ...
        batch.total_cards = sum(row.normalized.get("quantity", 0) for row in values if row.status == "valid")
...
def confirm_import(actor, batch_id, *, mapping_digest, version):
    ...
    check_version(batch, version)
    require(batch.status == "validated" and batch.error_rows == 0 and batch.total_rows > 0 and 0 < batch.total_cards <= 100000, "import_errors", "请处理全部异常，单次开卡量不超过10万张，可分批继续")
...
def save_format(actor, batch_id, *, name):
    batch = get_import(actor, batch_id, lock=True)
    require(actor.organization.kind in RESOURCE_KINDS and batch.status == "confirmed", "unconfirmed_format", "请先确认本机构列对应关系", 400)
    require(isinstance(name, str) and 0 < len(name.strip()) <= 100, "invalid_name", "请填写格式名称", 400)
    config = batch.configuration
    ...
    structure = {"headers": normalized_headers, "field_headers": field_headers, "header_row": config["header_row"], "quantity_mode": config["quantity_mode"]}
    item = ImportFormat.objects.create(organization=actor.organization, name=name.strip(), structure=structure)
...
def suggest_mapping(headers, stored_format=None):
    ...
    if stored_format:
        require(sorted(keys) == sorted(stored_format.structure["headers"]), "format_changed", "表头结构已变化，请重新对应列", 400)
        aliases = {field: {header} for field, header in stored_format.structure["field_headers"].items()}
