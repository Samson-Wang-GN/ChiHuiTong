...
class NotificationTests(TestCase):
    ...
    def enable(self, code="appointment.new"):
        ...
        return notifications.configure_template(self.platform, code, content=template.content,
            parameter_names=template.parameter_names, provider_template_id="100001", sign_name="合成测试",
            status="active", version=template.version, reason="合成报备配置")

    def test_template_platform_only_and_parameter_allowlist(self):
        template = self.enable()
        with self.assertRaises(BusinessError):
            notifications.configure_template(self.channel, template.code, content=template.content,
                parameter_names=template.parameter_names, provider_template_id="100001", sign_name="合成",
                status="active", version=template.version, reason="越权")
        with self.assertRaises(BusinessError):
            notifications.configure_template(self.platform, template.code, content="任意客户{phone}",
                parameter_names=["phone"], provider_template_id="100001", sign_name="合成",
                status="active", version=template.version, reason="变量越界")
    ...
    def test_read_notification_does_not_confirm_appointment_and_is_scoped(self):
        ...
        self.assertEqual(api_client(self.channel).post(f"/api/v1/notifications/{notice.id}/read").status_code, 404)
    ...
    def test_scheduler_24_72_and_expired_reschedule_requeues(self):
        ...
        with patch("django.utils.timezone.now", return_value=self.scheduled + timedelta(hours=70)):
            appointments.reschedule(item.id, proposed_at=self.scheduled + timedelta(days=10),
                                    version=item.version, customer_id=self.customer.id)
        for hours, expected in [(72, "success"), (95, "completed")]:
            with patch("django.utils.timezone.now", return_value=self.scheduled + timedelta(hours=hours)):
                scheduler.tick()
                ...
                Outbox.objects.exclude(kind="appointment.deadline").update(available_at=timezone.now() + timedelta(days=100))
                jobs.run_one()
            item.refresh_from_db()
            self.assertEqual(item.status, expected)

    def test_job_retry_is_platform_only_and_no_payload_leak(self):
        job = Outbox.objects.create(kind="sms.business", dedup_key="synthetic-job", payload={"phone": "13900000999"},
                                    status="failed", available_at=timezone.now(), last_error_code="sms_unavailable")
    ...
    def test_calendar_override_platform_only(self):
        ...
        result = client.post("/api/v1/calendar/2026-09-13", {"working": True, "note": "合成调休测试"}, format="json")
        self.assertEqual(result.status_code, 200)
        self.assertTrue(client.get("/api/v1/calendar/2026-09-13").data["working"])
        self.assertEqual(api_client(self.channel).get("/api/v1/calendar/2026-09-13").status_code, 403)

    @override_settings(TENCENT_SMS={"secret_id": "SYNTHETIC", "secret_key": "SYNTHETIC-SECRET", "sdk_app_id": "1400000001", "region": "ap-beijing"})
    def test_tencent_adapter_acceptance_and_failures(self):
        template = self.enable()
        raw = json.dumps({"Response": {"SendStatusSet": [{"Code": "Ok", "SerialNo": "SYNTHETIC-SERIAL", "PhoneNumber": "+8613900000099"}]}}).encode()
        with patch("urllib.request.OpenerDirector.open", return_value=io.BytesIO(raw)) as call:
            result = TencentSMS().send_template("13900000099", template, {"appointment_number": "SYNTH001"}, context="synthetic")
        ...
        with patch("urllib.request.OpenerDirector.open", return_value=io.BytesIO(b'{"Response":{"Error":{"Code":"Denied"}}}')):
            with self.assertRaises(BusinessError):
                TencentSMS().send_template("13900000099", template, {"appointment_number": "SYNTH001"}, context="synthetic")
...
class BillingNotificationTests(TestCase):
    ...
    def test_daily_reminder_once_overdue_once_never_auto_offline(self):
        with patch("django.utils.timezone.now", return_value=self.clock):
            scheduler.tick()
            scheduler.tick()
            self.assertEqual(Outbox.objects.filter(kind="sms.business", payload__isnull=False).exclude(dedup_key__startswith="appointment").filter(dedup_key__startswith="bill-sms").count(), 2)
        for delta in [1, 2]:
            with patch("django.utils.timezone.now", return_value=self.bill.due_at + timedelta(days=delta)):
                scheduler.tick()
        self.assertEqual(Outbox.objects.filter(dedup_key__startswith="bill-sms", dedup_key__endswith=":overdue").count(), 2)
