...
最新REQ-051：门诊开通支持保存待签草稿、自动生成PDF、下载打印、签署照片回传再提交审核；统一合同仅门诊/客户资源方/门诊渠道三类。平台先在“合同管理→合同模板配置”发布认可的单店/连锁模板；无模板可保存草稿但不能生成或提交。验证、部署和正式范本待验收边界见[合同生成交付记录](docs/contract-generation-review.md)、[TASK-110](docs/tasks.md)及[ADR-0033](docs/decisions/0033-generated-bilateral-clinic-contracts.md)。

2026-09-16：REQ-048～050正式后台已开发并部署应用`9b737ec`。统一合同管理、单店资料与合同一次提交/审核上线、连锁分步入驻及总部周/月账单、单店现付核销和恢复已接入真实服务端；197项后端测试及完整四角色Web回归通过。发布前后1098条核心记录ID摘要一致。现付错误核销退款延期，小程序本轮不改，真实微信商户/真机支付及生产高风险评审仍待完成。操作路径、证据和回退边界见[本轮交付记录](docs/contract-onboarding-review.md)，下方版本均为历史发布。

TASK-097～100（2026-09-13历史交付）：两个正式微信原生小程序代码位于`mini-programs/`，独立客户14页/门诊25页；当时只完成代码与服务器可执行测试，真实微信授权、原生编译/真机和发布待接入，详见[小程序开发交接](docs/mini-program-development.md)。该阶段未替换Web应用；最新Web部署以上方REQ-048～050为准，小程序尚未适配本轮新模式。
