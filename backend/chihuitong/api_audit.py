...
def scoped_object(actor, object_type, object_id):
    ...
    if object_type == "clinicprofilechange":
        ...
        item = ContractVersion.objects.filter(
            pk=object_id, contract__in=contracts.accessible_contracts(actor)
        ).first()
        ...
        return get_order(actor, object_id)
        ...
        query = (
            ...
            if actor.platform
            else Membership.objects.filter(organization=actor.organization)
        )
    elif object_type in {"product", "sourcebrand", "smstemplate"}:
        actor.require_platform()
        query = {"product": Product, "sourcebrand": SourceBrand, "smstemplate": SmsTemplate}[
            object_type
        ].objects.all()
    else:
            ...
            "results": [
                {
                    ...
                    or (
                        "客户本人"
                        if item.actor_role == "customer"
                        else "系统"
                        if not item.actor_id
                        else "历史未记录"
                    ),
