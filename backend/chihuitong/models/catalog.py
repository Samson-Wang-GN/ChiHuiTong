    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                condition=Q(redemption_units__gt=0), name="positive_redemption_units"
            ),
            models.CheckConstraint(condition=Q(validity_days__gt=0), name="positive_validity_days"),
            models.CheckConstraint(
                condition=Q(status__in=["active", "disabled"]), name="product_state"
            ),
        ]
...
class ProductRevision(Entity):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["product", "revision"], name="product_revision")
        ]
...
class SourceBrand(Entity):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["organization", "name"], name="brand_per_resource")
        ]
...
class Clinic(Entity):
    ...
    channel = models.ForeignKey(
        Organization, on_delete=models.PROTECT, related_name="managed_clinics"
    )
    responsible = models.ForeignKey(
        Membership, on_delete=models.PROTECT, related_name="assigned_clinics"
    )
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                condition=Q(confirmation_hours__gt=0), name="positive_confirm_hours"
            ),
            models.CheckConstraint(
                condition=Q(service_status__in=["online", "offline", "paused", "exited"]),
                name="clinic_service_state",
            ),
        ]
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["asset", "object_type", "object_id"], name="unique_attachment_link"
            )
        ]
...
class ClinicProfileChange(Entity):
    ...
    reviewed_by = models.ForeignKey(
        Membership, null=True, on_delete=models.PROTECT, related_name="profile_reviews"
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["clinic"], condition=Q(status="pending"), name="one_pending_profile"
            )
        ]
...
class Contract(Entity):
    organization = models.OneToOneField(
        Organization, on_delete=models.PROTECT, related_name="contract"
    )
...
class ContractVersion(Entity):
    ...
    channel = models.ForeignKey(
        Organization, null=True, on_delete=models.PROTECT, related_name="tripartite_versions"
    )
    ...
    reviewed_by = models.ForeignKey(
        Membership, null=True, on_delete=models.PROTECT, related_name="contract_reviews"
    )
    reviewed_at = models.DateTimeField(null=True)
    reason = EncryptedTextField(default="")
    ...
    class Meta:
        constraints = [
            ...
            models.UniqueConstraint(
                fields=["contract"], condition=Q(status="pending"), name="one_pending_contract"
            ),
            models.CheckConstraint(condition=Q(ends_at__gt=F("starts_at")), name="contract_dates"),
            models.CheckConstraint(
                condition=Q(settlement_cycle__in=["weekly", "monthly"]), name="contract_cycle"
            ),
        ]
...
class ContractProduct(Entity):
    contract_version = models.ForeignKey(
        ContractVersion, on_delete=models.PROTECT, related_name="products"
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["contract_version", "product"], name="one_product_per_version"
            ),
            ...
            models.CheckConstraint(
                condition=Q(mode="amount") | Q(value__lte=100), name="split_percent_limit"
            ),
        ]
