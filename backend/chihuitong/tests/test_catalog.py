...
class FileDetailsTests(TestCase):
    ...
    def test_metadata_keeps_same_file_access_boundary(self):
        ...
        asset = files.upload_file(
            owner, data=image_bytes(), filename="合成门诊照片.png", purpose="cover"
        )
        ...
        self.assertEqual(
            api_client(other).get(f"/api/v1/files/{asset.id}/details").status_code, 404
        )
...
class CatalogTests(TestCase):
    ...
    def test_fee_validation_ignores_superseded_terms_but_blocks_invalid_price_change(self):
        ...
        term = contracts.save_term(
            self.platform,
            self.channel_contract.id,
            product_id=self.product.id,
            mode="percent",
            value="80",
            version=1,
    ...
    def test_cooperation_combines_readonly_contract_and_products_with_state_tabs(self):
        newer = contract_fixture(
            self.platform, self.resource.organization, product=self.product, start=timezone.now()
        )
        response = api_client(self.resource).get(
            f"/api/v1/organizations/{self.resource.organization.id}/cooperation"
        )
        ...
        response = api_client(self.resource).get(
            f"/api/v1/organizations/{self.resource.organization.id}/contracts",
            {"status": "superseded"},
        )
        self.assertEqual(response.status_code, 200, response.data)
        ...
        self.assertEqual(
            api_client(self.channel)
            .get(f"/api/v1/organizations/{self.resource.organization.id}/cooperation")
            .status_code,
            404,
        )
    ...
    def test_future_contract_approval_checks_combined_allocation(self):
        ...
        future = (
            ContractVersion.objects.filter(contract__organization=self.resource.organization)
            .order_by("-revision")
            .first()
        )
