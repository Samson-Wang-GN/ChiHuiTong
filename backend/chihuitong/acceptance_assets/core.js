(function () {
  ...
  C.FormDialog = function({title,fields,initial={},hint,onSubmit,onClose,submitText='保存',width=760}) {
    ...
    async function submit(){let values;try{values=await form.validate();}catch{return;}const payload=JSON.stringify(values);if(retry.current.payload && retry.current.payload!==payload)retry.current.key=crypto.randomUUID();retry.current.payload=payload;setBusy(true);setError(null);try{await onSubmit(values,retry.current.key);C.refresh();A.Message.success('操作已完成');onClose();}catch(e){setError(e);}finally{setBusy(false);}}
