...
2026-09-16：新增[REQ-048 / 需求0.33](docs/requirements.md)，本轮仅完成文档。新门诊采用平台—签约主体双方合同：连锁总部覆盖多店、统一周/月后付；单店前台工作手机核销现付，付款责任仍属签约主体，无负责人代付。现付错误核销退款明确延期，已结清不允许撤销；支付技术异常保留核查/恢复。模型、合同上线、账单、权限、Web/小程序及原型尚待实施，见[TASK-101～107](docs/tasks.md)和[ADR-0031](docs/decisions/0031-clinic-subjects-and-payment-modes.md)。以下已开发/已测试记录仅指变更前基线，不能视为新规则已经上线；本轮未运行测试或部署。

TASK-097～100：两个正式微信原生小程序代码位于`mini-programs/`，不再只有浏览器演示。独立客户14页/门诊25页；缺AppID阶段只完成代码与服务器可执行测试，真实微信授权、原生编译/真机和发布待账号就绪，详见[小程序开发交接](docs/mini-program-development.md)。当前Web验收应用仍为df52809，本轮没有替换服务或修改验收数据。
