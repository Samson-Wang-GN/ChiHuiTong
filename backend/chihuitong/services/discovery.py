...
def eligible_clinics(product_id):
    ...
    qs = (
        Clinic.objects.select_related("organization")
        .filter(
            organization__status="active",
            channel__status="active",
            service_status="online",
            review_status="approved",
            profile_version__gt=0,
            products__product_id=product_id,
            products__status="online",
            products__product__status="active",
        )
        .annotate(
    ...
    return (
        qs.filter(clinic_contract__in=valid.values("id"), channel_contract__in=valid.values("id"))
        .annotate(
            authorized=Exists(
                ContractProduct.objects.filter(
                    contract_version_id=OuterRef("channel_contract"),
...
def owned_benefit(customer, benefit_id, *, bookable=True):
    benefit = (
        Benefit.objects.select_related("card__order")
        .filter(pk=benefit_id, customer=customer)
        .first()
    )
    require(benefit, "not_found", "权益不存在", 404)
    if bookable:
        require(
            benefit.card.status == "active"
            and not benefit.card.frozen
            and benefit.activated_at
            and benefit.expires_at
            and timezone.localtime(benefit.expires_at).date() >= timezone.localdate()
...
def listing(customer, benefit_id, *, longitude=None, latitude=None, query=""):
    ...
    if longitude is not None:
        ...
        distance = RawSQL(
            ...
            (float(latitude), float(latitude), float(longitude)),
...
def projection(clinic):
    ...
    return {
        ...
        **{
        ...
        "distance_m": round(clinic.distance_m)
        if getattr(clinic, "distance_m", None) is not None
        else None,
