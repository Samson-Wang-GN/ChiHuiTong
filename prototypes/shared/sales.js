...
window.PrototypeSales = (() => {
  ...
  function inspect(items,p){
    const known=rows(p,'salesCustomers'),seen=new Map();
    return items.map((r,i)=>{
      ...
      x.error=!x.name?'姓名必填':!/^1[3-9]\d{9}$/.test(x.phone)?'手机号格式不正确':!integer(x.quantity)||x.quantity>500?'数量须为1～500的整数':found&&found.name!==x.name?'手机号与姓名不一致':seen.has(x.phone)?(seen.get(x.phone)!==x.name?'同表手机号与姓名冲突':'同表手机号重复，请合并数量'):'';
      seen.set(x.phone,x.name);x.status=x.error?'数据异常':found?'匹配已有客户':'新建未绑定客户';return x;
    });
  }
  function seed(role){
  ...
  async function readExcel(file){
    if(!/\.xlsx$/i.test(file.name)||file.size>1024*1024)throw Error('原型支持单工作表.xlsx模板，文件不超过1MB；请勿上传真实客户资料。');
    ...
    if(end<0)throw Error('无法读取Excel文件，请使用下载的模板。');
    let offset=view.getUint32(end+16,true);const count=view.getUint16(end+10,true),entries=new Map();if(count>100)throw Error('工作簿过于复杂，请使用模板。');
    for(let i=0;i<count;i++){
      if(offset+46>bytes.length||view.getUint32(offset,true)!==0x02014b50)throw Error('Excel结构损坏。');
      const n=view.getUint16(offset+28,true),extra=view.getUint16(offset+30,true),comment=view.getUint16(offset+32,true),path=decoder.decode(bytes.slice(offset+46,offset+46+n));
      entries.set(path,{method:view.getUint16(offset+10,true),size:view.getUint32(offset+20,true),raw:view.getUint32(offset+24,true),local:view.getUint32(offset+42,true)});offset+=46+n+extra+comment;
    }
    const sheets=[...entries.keys()].filter(n=>/^xl\/worksheets\/sheet\d+\.xml$/.test(n));if(sheets.length!==1)throw Error('仅支持一个工作表，请使用模板。');
    async function xml(path){
      const e=entries.get(path);if(!e)return null;if(e.raw>2*1024*1024||e.local+30>bytes.length)throw Error('解压内容过大或文件损坏。');
      const start=e.local+30+view.getUint16(e.local+26,true)+view.getUint16(e.local+28,true);if(start+e.size>bytes.length)throw Error('文件内容不完整。');
      let content=bytes.slice(start,start+e.size);
      if(e.method===8){const reader=new Blob([content]).stream().pipeThrough(new DecompressionStream('deflate-raw')).getReader(),chunks=[];let size=0;try{for(;;){const r=await reader.read();if(r.done)break;size+=r.value.length;if(size>2*1024*1024)throw Error('解压内容超过原型上限。');chunks.push(r.value);}}finally{await reader.cancel();}content=new Uint8Array(size);let at=0;for(const c of chunks){content.set(c,at);at+=c.length;}}
      else if(e.method!==0)throw Error('不支持该压缩格式，请使用模板。');
      const text=decoder.decode(content);if(/<!DOCTYPE|<!ENTITY/i.test(text))throw Error('模板不允许外部实体。');const doc=new DOMParser().parseFromString(text,'application/xml');if(doc.querySelector('parsererror')||doc.getElementsByTagName('f').length)throw Error('文件损坏或包含公式，请改为纯文本/数值。');return doc;
    }
    const shared=await xml('xl/sharedStrings.xml'),strings=shared?[...shared.getElementsByTagName('si')].map(x=>x.textContent):[],doc=await xml(sheets[0]);
    const table=[...doc.getElementsByTagName('row')].map(row=>{const cells=[];for(const c of row.getElementsByTagName('c')){const ref=c.getAttribute('r')||'';let index=0;for(const char of ref.replace(/\d/g,''))index=index*26+char.charCodeAt(0)-64;if(index<1||index>20)throw Error('列数超出模板范围。');const v=c.getElementsByTagName('v')[0]?.textContent||'';cells[index-1]=c.getAttribute('t')==='s'?strings[Number(v)]:c.getAttribute('t')==='inlineStr'?c.textContent:v;}return cells;}).filter(r=>r.some(v=>v));
    if(table.length<2||table.length>201||['客户编号','姓名','手机号','开卡数量'].some((x,i)=>table[0][i]!==x))throw Error('模板须包含客户编号、姓名、手机号、开卡数量四列，且为1～200位客户。');
    return table.slice(1).map((r,i)=>({id:'ROW-'+(i+1),customerNo:r[0]||'',name:r[1]||'',phone:r[2]||'',quantity:Number(r[3])}));
  }
  function Wizard({pages,save,onClose}){
    const [step,setStep]=useState(0),[draft,setDraft]=useState({mode:physical,input:'批量客户',quantity:100,perCard:1,price:0,productId:'',source:'',name:'',phone:'',customerNo:'',customerQuantity:1}),[items,setItems]=useState([]),[file,setFile]=useState(''),[error,setError]=useState(''),[reading,setReading]=useState(false),[submitting,setSubmitting]=useState(false);
    ...
    return h(Drawer,{visible:true,width:900,title:'新建推广产品销售',maskClosable:false,footer:h('div',{className:'flow-actions'},step>0&&btn('上一步',()=>setStep(step-1)),step<2?btn('下一步',next,{type:'primary',disabled:reading}):btn('提交销售订单',submit,{type:'primary',disabled:submitting||reading})),onCancel:()=>Modal.confirm({title:'放弃未提交订单？',content:'关闭将放弃本次录入，已提交订单不受影响。',onOk:onClose})},
      ...
      step===1&&(draft.mode===physical?h(React.Fragment,null,note('无需客户资料或Excel，系统审核开卡时自动分配唯一演示号段。'),W.field('开卡张数',h(InputNumber,{value:draft.quantity,onChange:v=>put('quantity',v),min:1,max:500,precision:0}))):h(React.Fragment,null,draft.input==='单个客户'?h(React.Fragment,null,...[['customerNo','客户编号'],['name','客户姓名'],['phone','客户手机号']].map(([k,label])=>W.field(label,h(Input,{value:draft[k],onChange:v=>put(k,v)}))),W.field('客户开卡张数',h(InputNumber,{value:draft.customerQuantity,onChange:v=>put('customerQuantity',v),min:1,max:500,precision:0}))):h('div',{className:'flow-actions'},btn('下载客户Excel模板',()=>PrototypeExcel.download('记名销售客户模板',[{name:'客户清单',rows:[['客户编号','姓名','手机号','开卡数量'],['C001','客户甲（演示）','13800000011',2],['C002','客户乙（演示）','13800000012',1]]}])),h(Upload,{autoUpload:false,showUploadList:false,beforeUpload:upload,accept:'.xlsx',disabled:reading},btn(reading?'正在读取':'上传客户Excel')),btn('使用演示客户名单',()=>{setFile('演示客户.xlsx');setItems([{name:'客户甲（演示）',phone:'13800000011',quantity:2},{name:'客户乙（演示）',phone:'13800000012',quantity:1}]);}),h('span',null,file)),note('仅使用虚构资料；原型限1MB单工作表.xlsx，1～200位客户，总卡数不超过500。'),h(W.RecordList,{items:checked,states:['匹配已有客户','新建未绑定客户','数据异常'],columns:[['name','客户'],['phone','手机号'],['quantity','开卡张数'],['status','校验结果'],['error','异常原因']]}))),
      step===2&&h(React.Fragment,null,W.field('推广产品',h(Select,{value:draft.productId,options:catalog(pages).filter(p=>authorized(pages,p.id,PROTOTYPE.org)).map(p=>({label:p.name,value:p.id})),onChange:v=>put('productId',v)})),W.field('来源展示名',h(Select,{value:draft.source,options:sources(pages,PROTOTYPE.org),onChange:v=>put('source',v)})),W.field('每卡权益份数',h(InputNumber,{value:draft.perCard,onChange:v=>put('perCard',v),min:1,max:100,precision:0})),W.field('单卡采购价（元）',h(InputNumber,{value:draft.price,onChange:v=>put('price',v),min:0,precision:2})),h(Descriptions,{column:1,border:true,data:[{label:'销售方式',value:draft.mode},{label:'开卡张数',value:quantity},{label:'权益总份数',value:quantity*draft.perCard},{label:'采购总金额',value:money(quantity*unitCents)+'元'},{label:'付款关系',value:'本资源方向平台付款，不是客户付款或门诊获客费'}]}),note(unitCents===0?'零元采购：提交后平台审核，审核通过开卡。':'收费采购：提交订单后付款，平台确认全额到账才可审核开卡。')));
  }
