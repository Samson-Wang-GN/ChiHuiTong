...
def edit_draft(actor, version_id, *, data, version, reason):
    ...
    advance(item, *data)
    link_files(data["attachment_ids"], item.contract)
    audit(actor, item.contract, "contract.draft_updated", reason=reason, version_id=str(item.id))
    return item


@transaction.atomic
def terminate(actor, version_id, *, version, reason):
    actor.require_platform()
