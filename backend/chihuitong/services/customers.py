...
def normalize_customer(raw):
    ...
    if age is not None and age != "":
        if isinstance(age, str) and age.isdigit():
            age = int(age)
        require(
            type(age) is int and 0 <= age <= 150,
            "invalid_age",
            "年龄须为0～150整数，请修正或不导入该列",
            400,
        )
        result["age"] = age
    ...
    if occupation is not None and occupation != "":
        require(
            isinstance(occupation, str) and len(occupation.strip()) <= 100,
            "invalid_occupation",
            "职业文本过长，请修正或不导入该列",
            400,
        )
...
def event(card, kind, *, obj=None, at=None, **payload):
    return DomainEvent.objects.create(
        card=card,
        kind=kind,
        object_id=obj.id if obj else card.id,
...
def claim(customer_id, card_id):
    ...
    customer = (
        ...
        .filter(pk=customer_id, registered_at__isnull=False)
        .first()
    )
    require(
        customer and card.customer_id == customer.id, "not_found", "权益不存在或不属于当前客户", 404
    )
    require(order.mode == "named", "invalid_claim", "实体卡请扫码激活")
    if card.activated_at:
        return Benefit.objects.get(card=card)
    require(
        order.status == "issued" and card.status == "unclaimed" and not card.frozen,
        "claim_unavailable",
        "当前权益暂不可领取",
    )
...
def activate(customer_id, credential):
    require(
        isinstance(credential, str) and 20 <= len(credential) <= 200,
        "invalid_card",
        "卡片凭证不合法",
        400,
    )
    ...
    source, _ = CustomerSource.objects.get_or_create(
        customer=customer, organization=order.organization
    )
    now = timezone.now()
    benefit = Benefit.objects.create(
        ...
        available=order.units_per_card,
        activated_at=now,
        expires_at=now + timedelta(days=order.validity_days),
    )
...
def update_profile(customer_id, data):
    require(
        isinstance(data, dict) and not (set(data) - {"name", "gender", "age", "occupation"}),
        "invalid_fields",
        "仅可修改姓名、性别、年龄和职业，不可修改手机号",
        400,
    )
    customer = (
        Customer.objects.select_for_update()
    ...
    normalized = normalize_customer(
        {"name": data.get("name", customer.name), "phone": customer.phone, "quantity": 1, **data}
    )
    profile = dict(customer.profile)
    for key in ["gender", "age", "occupation"]:
        if key in data:
            profile[key] = normalized.get(key)
            profile.setdefault("field_sources", {})[key] = {
                "source": "customer",
